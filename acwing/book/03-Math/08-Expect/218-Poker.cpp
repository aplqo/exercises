...
struct State {
  ...
  {
    return maxp * 4 - accumulate(cnt, cnt + 4, 0u) + (king[0] == 4) +
           (king[1] == 4);
  }
  ...
  bool succ() const
  {
    for (unsigned int i = 0; i < 4; ++i)
      if (cnt[i] + (king[0] == i) + (king[1] == i) < ex[i]) return false;
    return true;
  }
};
...
real dfs(const State& s)
{
  ...
  for (unsigned int i = 0; i < 4; ++i)
    if (s.cnt[i] < maxp) s[m] += dfs(s.takeNormal(i)) * (maxp - s.cnt[i]) / sm;
  for (unsigned int i = 0; i < 2; ++i) {
    if (s.king[i] != 4) continue;
    real t = 1000;
