        ...
        for (unsigned int j = 0; j < 10; ++j)
        {
            const unsigned long long mv = [](unsigned int cd, int j, int lst) {
                if (lst == 3)
                    ...
                    return accumulate(f[cd - 1] + max(3 - lst - (j == 6), 0), f[cd - 1] + 4, 0ull);
                else
                    return f[cd - 1][3];
            }(cd, j, lst);
            res[i] = j;
...
int main()
{
    ...
    for (unsigned int i = 0; i < t; ++i)
    {
        ...
        cin >> n;
        transform(res, res + dp(n, res), ostreambuf_iterator<char>(cout), [](unsigned int i) { return i + '0'; });
        cout.put('\n');
    }
    return 0;
}