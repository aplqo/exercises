...
struct Point
{
    Point(const Cost x, const Cost y)
        : x(x)
        , y(y) {};
    Point(const unsigned int i)
        : Point(X(i), Y(i)) {};
    inline Vertex operator-(const Point b) const { return Vertex { x - b.x, y - b.y }; }
    Cost x, y;
};
...
static void init(const unsigned int n)
{
    ...
    const auto ri = reverse_iterator<Cost*>(c + n + 1);
...
Cost dp(const unsigned int n, const Cost s)
{
    ...
    for (unsigned int i = n; i; --i)
    {
        ...
        {
            const Point cp(i);
            while (back - front > 1 && (cp - Point(*(back - 2))) * (cp - Point(*(back - 1))) >= 0)
                --back;
            *(back++) = i;
        }
    }
