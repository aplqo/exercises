...
void PrintTest(const unsigned int id)
{
    cout << "Test " << id << ": ";
}
...
void ReadOut(ifstream& out)
{
    ...
    sscanf(buf, "Best jury has value %d for prosecution and value %d for defence:", &op, &od);
...
void ReadAns(ifstream& ans)
{
    ...
    ans.ignore(3, '\n');
    ans.getline(buf, 100, '\n');
    sscanf(buf, "Best jury has value %d for prosecution and value %d for defence:", &ansp, &ansd);
}
...
void TestOrder(const unsigned int id)
{
    for (unsigned int i = 1; i < m; ++i)
        ...
        {
            PrintTest(id);
            cout << "Wrong order " << sel[i] << " should print before " << sel[i - 1] << endl;
            exit(Order);
        }
}
