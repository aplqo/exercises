...
inline void CreateGraph(const unsigned int n, const unsigned int m,
                        const unsigned int v, const unsigned int s)
{
...
inline void Solve()
{
  ...
  cin >> n >> m >> s >> v;
  if (cin.fail()) exit(0);
  for (auto i = gop; i < gop + n; ++i)
    cin >> i->x >> i->y;
  for (auto j = hole; j < hole + m; ++j)
    cin >> j->x >> j->y;
  CreateGraph(n, m, v, s);
