...
class SegmentTree {
 public:
  void build(const unsigned int l, const unsigned int r, const int v[],
             SegmentTree*(&cur));
...
void SegmentTree::update(const unsigned int l, const unsigned int r,
                         const int delt)
{
  if (delt < fst.current.val) return;
  if (rv - lv == 1) {
    fst.current.val = delt;
    fst.history.val = std::min(fst.history.val, fst.current.val);
    return;
...
{
  if (l == lv && r == rv) return History ? fst.history.val : fst.current.val;
  pushdown();
  const unsigned int mv = (lv + rv) >> 1;
  if (r <= mv)
...
inline void SegmentTree::pushdown()
{
  if (fst.isDirty() || snd.isDirty()) {
    const auto doPush =
        [this, mv = std::min(this[lc].fst.current.val,
                             this[rc].fst.current.val)](SegmentTree* p) {
...
inline void SegmentTree::pushup()
{
  ...
  if (mn->fst.current.val > mx->fst.current.val) swap(mn, mx);
  fst.current.val = mn->fst.current.val;
  snd.current.val =
      std::min((mx->fst.current.val == fst.current.val ? &mx->snd : &mx->fst)
                   ->current.val,
               mn->snd.current.val);
...
int main()
{
  ...
  for (unsigned int i = m; i; --i) {
    const unsigned int op = readInt<unsigned int>(),
                       l = readInt<unsigned int>(), r = readInt<unsigned int>();
    switch (op) {
      case 1:
