...
class SegmentTree {
 public:
  ...
  void assign(const SegmentTree* const old, const Range cur, const Range qry,
              const Train& val);
 ...
 private:
  inline void pushTag(const SegmentTree* old, const Range node,
                      const Train& tag);
...
void SegmentTree::assign(const SegmentTree* const old, const Range node,
                         const Range query, const Train& v)
{
  ...
  const auto subtreeAssign = [this, query](const SegmentTree* old,
                                           const Range node, const Train& tag,
                                           const Train& v) {
    SegmentTree* tp = cur++;
    if (!tag.time)
  ...
  const auto fromOld = [this](const SegmentTree* old, const Range node,
                              const Train& tag) -> const SegmentTree* {
    if (!tag.time) return old;
    SegmentTree* tp = cur++;
...
{
  if (node.r - node.l == 1 || val.time) return val;
  const unsigned int mv = (node.l + node.r) >> 1;
  if (pos < mv)
...
inline void SegmentTree::pushTag(const SegmentTree* old, const Range node,
                                 const Train& tag)
{
  lc = old->lc;
  rc = old->rc;
...
inline void pop(const unsigned int time, const Range full,
                const unsigned int pos)
{
  ...
  if (!v.time)
    ...
    (SegmentTree::cur++)
        ->assign(root[time - 1], full, Range{pos, pos + 1},
                 root[v.time - 1]->get(full, pos));
  }
}
inline void push(const unsigned int time, const Range full, const Range query,
...
int main()
{
  ...
  const auto decode = [n, ty, &ans](unsigned int v) {
    return (v + ty * ans) % n + 1;
  };
