...
char inbuf[maxn * 20 + maxm * 20 + 1000], *inpos = inbuf, outbuf[maxm * 20 + 100], *outpos = outbuf;
...
{
public:
    void build(const unsigned int l, const unsigned int r, const long long v[], SegmentTree*(&cur));
    inline void add(const unsigned int l, const unsigned int r, const long long v) { applyTag(l, r, { v, ninf }); }
    inline void minus(const unsigned int l, const unsigned int r, const long long v) { applyTag(l, r, Tag { -v, 0 }); }
    inline void assign(const unsigned int l, const unsigned int r, const long long v) { applyTag(l, r, Tag { ninf, v }); }
    inline long long get(const unsigned int pos) { return query<Operate::Max>(pos); }
    inline long long historyMax(const unsigned int pos) { return query<Operate::HistoryMax>(pos); }
...
void SegmentTree::build(const unsigned int l, const unsigned int r, const long long v[], SegmentTree*(&cur))
{
    lv = l;
    rv = r;
    mv = (l + r) >> 1;
...
void SegmentTree::applyTag(const unsigned int l, const unsigned int r, const Tag& t)
{
    if (rv - lv == 1)
    {
...
{
    if (rv - lv == 1)
        return op == Operate::Max ? current.val : history.val;
    pushdown();
