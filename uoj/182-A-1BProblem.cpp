...
{
#define decl(x)                                                                                                          \
    inline friend Number operator x(const Number l, const Number r) { return Number { ((l.val + mod) x r.val) % mod }; } \
...
namespace Polynomial
{
    ...
    void convolution(const ConstArrayPtr& a, const ConstArrayPtr& b, const ArrayPtr& dest, Func pred)
    {
    ...
    namespace Evaluation
    {
        ...
        void evaluation(const ConstArrayPtr& a, const ConstArrayPtr& pos, Number dest[])
        {
            ...
            Product::product<false>(pos, lgn, prod, cur, [](Number* cur, Number v) { cur[0] =Number {1}; cur[1] = -v; });
            {
...
void answerFrac(const unsigned int n, const unsigned int q, const Number pc[], const unsigned int qid[])
{
    if (!q)
        return;
    using namespace Polynomial;
    static Number tmem[maxp + 10], dmem[maxp + 10], tval[maxp + 10], dval[maxp + 10];
    init(std::max(n, q));
    ArrayPtr prd { tmem }, dprd { dmem };
    const ConstArrayPtr pos { pc, q };
    Product::product(ConstArrayPtr { a, n }, prd, [](Number* cur, Number b) { cur[0] = b; cur[1] = Number {1}; });
    derivative(prd, dprd);
    Evaluation::evaluation(prd, pos, tval);
    Evaluation::evaluation(dprd, pos, dval);
    getModInv(tval, tval, q);
    for (unsigned int i = 0; i < q; ++i)
        ans[qid[i]] = trans[qid[i]].a * Number { n } + trans[qid[i]].b * tval[i] * dval[i];
}
...
int main()
{
    ...
    answerQuery(n, m);
    std::copy(ans + 1, ans + 1 + m, std::ostream_iterator<Number>(std::cout, "\n"));
    return 0;
}