...
enum class dir : int { W = 0, S = 1, E = 2, N = 3 };
const int dx[4] = {0, 1, 0, -1}, dy[4] = {-1, 0, 1, 0};
struct node {
  node(int x, int y, int d) : x(x), y(y), d(d){};
  int x, y, d;
  inline node turn(int di) { return node(x, y, (d + di + 4) % 4); }
  template <class T>
...
inline void bfs()
{
  ...
  while (!que.empty()) {
    ...
    for (int i = 1; i <= 3; ++i) {
      ...
      if (nx < 1 || ny < 1 || nx >= n || ny >= m || maz[nx][ny]) break;
...
inline void read()
{
  ...
  for (int i = 0; i <= n; ++i)
    for_each(ans[i], ans[i] + 1 + m,
             [](int(&t)[4]) -> void { fill(t, t + 4, INT_MAX); });
}
