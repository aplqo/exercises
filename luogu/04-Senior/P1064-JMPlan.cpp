...
struct obj {
  obj() = default;
  inline obj(int v, int w, int id) : v(v), w(w * v), id(id){};
  int v, w, id;
  int ans[maxn] = {};
};
struct node : obj {
  node() = default;
  node(int v, int w, int id) : obj::obj(v, w, id){};

  inline void AddChild(int v, int w, int id) { child[cnt++] = obj(v, w, id); }
  inline void solve(int nv)
  {
    ...
    const int cv = nv - v;
    {
      ...
      if (cv >= c.v) c.ans[nv] = max(c.ans[nv], pre->ans[cv - c.v] + w + c.w);
    }
...
int main()
{
  ...
  for (node* i = o; i < o + m + 1;) {
    i->nxt =
        find_if(i + 1, o + 1 + m, [](const node& i) -> bool { return i.id; });
    i->nxt->pre = i;
    i = i->nxt;
  }
