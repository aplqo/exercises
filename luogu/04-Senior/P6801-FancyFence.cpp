...
inline unsigned long long c2(const unsigned long long n, const unsigned long long v)
{
    return (n * v / 2) % mod;
}
...
{
    if (sec[x].lc)
        sec[x].w += dfs(sec[x].lc, sec[x].h);
    if (sec[x].rc)
        sec[x].w += dfs(sec[x].rc, sec[x].h);
    sec[x].w %= mod;
    ans = (ans + (c2(sec[x].h + fv + 1, sec[x].h - fv) * c2(sec[x].w + 1, sec[x].w) % mod)) % mod;
    return sec[x].w;
}
