...
real ca[maxn + 1], cb[maxn + 1], f[maxn + 1], A[maxn + 1], B[maxn + 1],
    Rate[maxn + 1];
...
class Convex {
 public:
  inline void push(const unsigned int x)
  {
    const Point cp(x);
    while (back - front >= 2 &&
           (cp - Point(*(back - 2))) * (cp - Point(*(back - 1))) >= -eps)
      --back;
    *(back++) = x;
  }
  inline real top(const unsigned int x)
  {
    ...
    while (back - front >= 2 &&
           cv * (Point(*(front + 1)) - Point(*front)) >= -eps)
      ++front;
...
void cdq(const unsigned int l, const unsigned int r)
{
  ...
  sort(tmp + mid, tmp + r,
       [](unsigned int a, unsigned int b) { return K(a) > K(b); });
  for (unsigned int* i = tmp + mid; i < tmp + r; ++i)
    f[*i] = max(f[*i], cv.top(*i));
  cv.clear();
  cdq(mid, r);
  inplace_merge(buf + l, buf + mid, buf + r,
