...
enum { Accept = 0, Sum = 1, Solution = 2, Output = 3, Increase = 4 };
template <class... Args>
...
unsigned long long testSum(ifstream& out, const char* ans)
{
  ...
  if (ansv != res) fail(Sum, "Sum not match! Read=", res, " Expected=", ansv);
  return ansv;
}
void testSeq(ifstream& out, const char* in, const long long ansv)
{
  ...
  for (unsigned int i = 2; i <= n; ++i) {
    ...
    fin >> av;
    if (!(nv > lst))
      fail(Increase, "Output sequence not inceease. b[", i - 1, "]=", lst,
           " b[", i, "]=", nv);
    acc += abs(av - nv);
    lst = nv;
  }
  if (acc != ansv)
    fail(Solution, "Output sequence sum not correct. Expect=", ansv,
         " Actually=", acc);
}
