...
template <bool chk>
void dfs(const unsigned int pos, const unsigned long long s,
         const unsigned long long acc = 1)
{
  if (s == 1) {
    ans.push_back(acc);
    return;
  ...
  if constexpr (chk) {
    if (s > primes[pos] && isPrime(s - 1)) ans.push_back(acc * (s - 1));
  }
  if (po[pos][2] > s) return;
