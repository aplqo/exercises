...
#define decl(x)                                                   \
  ...
  decl(*);
#undef decl
  inline Number square() const { return val * val; }
...
inline Number cubeSum(const Number n) { return (inv2 * n * (n + 1)).square(); }
Number dujiao(const unsigned long long n)
{
  if (n <= maxb) return sumG[n];
  if (const auto it = mem.find(n); it != mem.end()) return it->second;
