...
void dp(const int sx, const int sy, const int ex, const int ey, const int k)
{
    ...
    for (int i = sx; i != ex + sdx; i += sdx)
        for (int j = sy; j != ey + sdy; j += sdy)
            res[i][j] = max({ res[i - sdx][j], res[i][j - sdy], square(i - (k - 1) * sdx, j - (k - 1) * sdy, i, j) });
}
unsigned int getMax(const int n, const int m, const int k)
{
    ...
    for (int i = 1; i <= n; ++i)
        ...
        {
            ret = max({ ret,
                squ[DR][i][j] + max(squ[DL][n][j + 1] + squ[UR][i + 1][j], squ[UL][i + 1][1] + squ[DL][i][j + 1]),
                squ[DL][i][j] + max(squ[UL][i + 1][1] + squ[DR][i][j - 1], squ[UL][i + 1][j] + squ[UR][1][j - 1]),
                squ[UR][i][j] + max(squ[DR][i - 1][m] + squ[UL][i][j + 1], squ[DL][n][j + 1] + squ[DR][i - 1][j]),
                squ[UL][i][j] + max(squ[DR][i - 1][m] + squ[UR][i][j - 1], squ[DR][n][j - 1] + squ[DL][i - 1][j]) });

            if (j - k >= k && j + k - 1 <= m - k + 1 && i - k >= 0)
                ret = max(ret, squ[DR][n][j - k] + squ[UL][1][j + 1] + square(i + 1 - k, j + 1 - k, i, j));
            if (i - k >= k && i + k - 1 <= n - k + 1 && j - k >= 0)
                ret = max(ret, squ[DR][i - k][m] + squ[UL][i + 1][1] + square(i + 1 - k, j + 1 - k, i, j));
        }
    return ret;
}
