...
unsigned int fa[maxln + 10][maxn], fb[maxln + 10][maxn],
    fs[maxln + 10][maxn + 10];
unsigned int pcnt[maxn + 10];

inline unsigned int fma(const unsigned long long v, const unsigned long long a,
...
void fmt(unsigned int dest[], const unsigned int lgn)
{
  ...
  for (unsigned int i = 0; i < lgn; ++i) {
    ...
    for (unsigned int j = 0; j <= msk; ++j)
      if (b & j) dest[j] = fma(dest[j], dest[j ^ b], v);
  }
}
