...
Number f(const unsigned int l, const unsigned int lst1, const unsigned int lst2, const bool suc, const bool up, const ZeroTyp zero)
{
    static const auto success = [](unsigned int i, unsigned int lst1, unsigned int lst2, ZeroTyp zero) -> bool {
    ...
    if (up)
        ret += f(l - 1, lst2, x[l], success(x[l], lst1, lst2, zero), true, nxt(zero, x[l]));
...
{
    unsigned int* p = find_if(v + 1, v + 1 + l, [](const unsigned int i) { return i; });
    fill(v + 1, p, 9);
    --(*p);
    return l - (!*p && p == v + 1 + l);
}
