                                            ...
                                            const unsigned int k)
{
  ...
  const auto id = [n](const unsigned int x, const unsigned int y,
                      const unsigned int dir) { return 2 * (x * n + y) + dir; };
  ...
  for (unsigned int i = 0; i < n; ++i)
    for (unsigned int j = 0; j < n; ++j) {
      ...
      if (i + 1 < n) addEdge(id(i, j, Out), id(i + 1, j, In), inf, 0);
      if (j + 1 < n) addEdge(id(i, j, Out), id(i, j + 1, In), inf, 0);
    }
