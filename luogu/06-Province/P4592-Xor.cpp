...
namespace Trie
{
    ...
    {
        static const auto subtree = [](const RootList& olds, const RootList& cur, const unsigned int cl) {
            return cur[0]->chl[cl]->siz + cur[1]->chl[cl]->siz
                - olds[0]->chl[cl]->siz - olds[1]->chl[cl]->siz;
        };
        unsigned int ret = 0;
        for (unsigned int msk = 1u << maxl; msk; msk >>= 1)
        {
...
int main()
{
    ...
    for (unsigned int i = 0; i < q; ++i)
    {
        ...
        else
        {
            ...
            cout << maxXor(RootList { dfsRoots[l], dfsRoots[Lca::fa[l]] }, RootList { dfsRoots[x], dfsRoots[y] }, z) << "\n";
        }
    }
    return 0;
}