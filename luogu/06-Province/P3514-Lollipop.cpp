...
{
  if ((s & 0x01) == part) return make_pair(s, Range{1, n});
  const auto p1 = find(a + 1, a + 1 + n, 1) - a;
  const auto p2 = find(reverse_iterator(a + n), reverse_iterator(a), 1) -
                  reverse_iterator(a + n) + 1;
  if (p1 - 1 < p2)
    return make_pair(s - (p1 - 1) * 2 - 1, Range{p1 + 1, n});
  else
    return make_pair(s - p2 * 2 - 1, Range{1, n - p2 - 1});
}
void moveRange(const unsigned int s, const Range ini)
{
  if (ini.l > ini.r) return;
  rg[s] = ini;
  for (Range* i = rg + s; i > rg + 1; i -= 2) {
    if (a[i->l] == 2)
      *(i - 2) = Range{i->l + 1, i->r};
    else if (a[i->r] == 2)
      *(i - 2) = Range{i->l, i->r - 1};
