...
static void init(const unsigned int n)
{
  ...
  for (unsigned int i = 1; i < n; ++i) {
    ...
    for (int j = cb; j >= 0; --j) {
      static const auto count = [](const unsigned int x, const unsigned int b,
...
unsigned int query(const unsigned int l, const unsigned int r)
{
  ...
  static const auto pred = [](pair<unsigned int, unsigned int> a,
                              pair<unsigned int, unsigned int> b) {
    return a.first != b.first ? a.first < b.first : a.second > b.second;
  };
  static const auto count = [](const unsigned int v, const unsigned int b,
                               const unsigned int e) {
  ...
  if (indl < indr) {
    ...
    ret.first = count(ret.second, indl, indr - 1) +
                (vis[ret.second] == sta ? val[ret.second] : 0);
