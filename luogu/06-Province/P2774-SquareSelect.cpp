...
const unsigned int maxv = (maxn + 1) * (maxn + 1) * 2, maxe = maxv * 2 + maxn * maxn * 2 + maxn * maxn * 4 * 2;
...
long long isap(const unsigned int s, const unsigned int t, const unsigned int cv)
{
...
pair<unsigned int, unsigned int> buildGraph(const unsigned int n, const unsigned int m)
{
    ...
    const auto id = [n](const unsigned int x, const unsigned int y, const unsigned int t) { return 2 * (x * n + y) + t; };
...
int main()
{
    ...
    cout << sum - isap(s, t, ml * ml * 2 + 2) << endl;
    return 0;
}