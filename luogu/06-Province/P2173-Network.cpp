...
enum Result { Success = 0, NotExist = 1, Cycle = 2, Degree = 3, NoPath };
const char* errStr[] = {"Success.", "No such edge.", "Error 2.", "Error 1."};
class LinkCutTree {
 public:
  ...
  pair<Result, unsigned int> minPath(const unsigned int x,
...
bool LinkCutTree::link(const unsigned int x, const unsigned int y)
{
  ...
  makeRoot(px);
  if constexpr (check) {
    if (findRoot(py) == px) return false;
  }
  px->fa = py;
  return true;
}
...
pair<Result, unsigned int> LinkCutTree::minPath(const unsigned int x,
                                                const unsigned int y)
{
  if (x == y) return make_pair(Success, snodes[x].val);
  SplayNode *const px = snodes + x, *const py = snodes + y;
  if (findRoot(py) != findRoot(px)) return make_pair(NoPath, 0);
  split(px, py);
...
inline void addEdge(unsigned int x, unsigned int y, const unsigned int col)
{
  ...
  ++degree[x][col];
  ++degree[y][col];
  if (!lct[col].link<false>(x, y)) throw 0;
}
Result changeColor(unsigned int u, unsigned int v, const unsigned int col)
{
  if (u > v) swap(u, v);
  const auto it = tree[u].find(v);
  if (it == tree[u].end()) return NotExist;
  if (it->second == col) return Success;
  if (degree[u][col] + 1 > 2 || degree[v][col] + 1 > 2) return Degree;
  if (!lct[col].link<true>(u, v)) return Cycle;
...
int main()
{
  ...
  for (unsigned int i = 0; i < k; ++i) {
    ...
    switch (op) {
      ...
      case 2:
        cin >> v3;
        const auto [suc, val] = lct[v1].minPath(v2, v3);
        if (suc == NoPath)
          cout << "-1";
        else
          cout << val;
