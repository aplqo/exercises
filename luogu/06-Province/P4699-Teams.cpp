...
static pair<unsigned int, unsigned int> dp(const unsigned int n)
{
    ...
    for (unsigned int i = a[1].member + 1; i <= n; ++i)
    {
        ...
        {
            team[i] = team[i - 1];
            maxTeam[i] = min(mnM, maxTeam[i - 1] + (1ull * team[i - 1] * (maxTeam[i - 1]) == i - 1));
        }
    }
...
static void printSolution(const unsigned int n, const unsigned int tem, const unsigned int mxT)
{
    ...
    for (unsigned int i = 1; i <= n; ++i)
    {
        if (teml[i].valid)
        {
            for_each(a + i, a + teml[i].end + 1, [ptr](const Child& i) { ptr->push_back(i.id); });
            ++ptr;
            i = teml[i].end;
            continue;
        }
