...
inline unsigned int fold(const unsigned int lval, const unsigned int rval,
...
class STree {
 public:
  ...
  {
    if (rv - lv == 1 || laz) return left;
    return pos < mv ? lc->find(pos) : rc->find(pos);
  }
  void replace(const unsigned int l, const unsigned int r,
...
unsigned int query(unsigned int x, unsigned int y)
{
  ...
  while (top[x] != top[y]) {
    if (dep[top[x]] > dep[top[y]]) {
      lval = fold(lval, SRoot->query(dfn[top[x]], dfn[x] + 1), left,
                  SRoot->find(dfn[x]));
      left = SRoot->find(dfn[top[x]]);
      x = fa[top[x]];
    }
    else {
      rval = fold(rval, SRoot->query(dfn[top[y]], dfn[y] + 1), right,
                  SRoot->find(dfn[y]));
      right = SRoot->find(dfn[top[y]]);
      y = fa[top[y]];
    }
  }
  const unsigned int v = dep[x] > dep[y] ? SRoot->query(dfn[y], dfn[x] + 1)
                                         : SRoot->query(dfn[x], dfn[y] + 1);
