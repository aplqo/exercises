...
memory f(const unsigned int l, const bool ln, const bool lm, const bool lk)
{
    ...
    for (unsigned int i = 0; i <= mi; ++i)
        for (unsigned int j = 0; j <= mj; ++j)
        {
            if (lk && b(k, l) > (i ^ j))
                continue;
            auto t = f(l - 1, ln && b(n, l) == i, lm && b(m, l) == j, lk && b(k, l) == (i ^ j));
            t.sum = (t.sum + (t.cnt * ((static_cast<unsigned long long>(i ^ j) << (l - 1)) % mod)) % mod) % mod;
            ret = ret + t;
        }
    vis[l][ln][lm][lk] = true;
...
int main()
{
    unsigned int t;
    cin >> t;
    static const auto d = [](const unsigned long long v) { return v ? log2(v) + 1 : 1; };
