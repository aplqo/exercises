...
inline bool test(num d)
{
    ...
    for (int i = 1; i <= n; ++i)
        for (edge* j = head[i]; j; j = j->pre)
            if (fla[j->to] == p - pl && j->val >= maxval - lstd)
                return true;
    return false;
}
...
int main()
{
    ...
    maxval = max_element(pl, pl + m, [](const plan& a, const plan& b) { return a.val < b.val; })->val;
    minval = maxval - max_element(ed, cur, [](const edge& a, const edge& b) -> bool { return a.val < b.val; })->val;
