...
inline unsigned int applyEvent(const Event* e, const int active) // return new active connections
{
...
void binary(const QRange er, const VRange vr)
{
    ...
    if (pl)
        binary(QRange { er.first, er.first + pl }, VRange { vr.first, mid });
    if (pr)
        binary(QRange { er.first + pl, er.second }, VRange { mid, vr.second });
}
