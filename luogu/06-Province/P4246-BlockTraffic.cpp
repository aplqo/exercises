...
struct vertex {
  vertex() = default;
  vertex(const unsigned int r, const unsigned int c) : r(r), c(c) {}
  unsigned int vertexNo() const { return r * (::c) + this->c; }
...
unsigned int getFather(const vertex x) { return getFather(x.vertexNo()); }
...
inline size_t stat() { return s.size(); }
}  // namespace unf
struct edge {
  edge() = default;
...
class STree {
 public:
  ...
  void dfs()
  {
    ...
    if (rv - lv == 1) {
      query& q = qry[lv];
      if (q.vis) q.ans = unf::getFather(q.a) == unf::getFather(q.b);
      unf::undo(s);
      return;
    }
    lc->dfs();
  ...
  while (true) {
    ...
    switch (op[0]) {
      ...
      case 'C': {
        ...
        if (ed[en].beg != lst) SRoot->modify(ed[en].beg, lst, en);
        ed[en].beg = maxc + 1;
        break;
      }
      case 'A':
