...
bool checkVerdict(const unsigned int id, ifstream& out, ifstream& fans)
{
    ...
    fans >> ex;
    if (c != ex)
        fail(id, "Wrong verdict. Expected=", ex, " Read=", c);
    return ex == 'Y';
}
void checkPermutation(const unsigned int id, const unsigned int n, ifstream& out)
{
    ...
    for (unsigned int i = 1; i < n; ++i)
    {
        ...
        if (typ[cur] == typ[lst])
            fail(id, "Wrong solution. p[", i - 1, "]=", lst, " p[", i, "]=", cur, " but type[", lst, "]=type[", cur, "]=", typ[cur]);
        lst = cur;
        vis[cur] = id + 1;
    }
}
...
int main(int, char* argv[])
{
    ...
    for (unsigned int i = 0; i < t; ++i)
    {
        ...
        if (checkVerdict(i, out, ans))
            checkPermutation(i, n, out);
    }
    return 0;
}