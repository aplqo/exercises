...
void dfs(const unsigned int x, const unsigned int fa)
{
    ...
    for (const Edge* i = head[x]; i; i = i->pre)
    {
        ...
        if (i->dir)
        {
            for (unsigned int i = siz[x]; i; --i)
            {
                for (unsigned int j = 1; j <= siz[to]; ++j)
                    f[x][i + j] += f[x][i] * (sum[to][siz[to]] - sum[to][j]) * g[i - 1][j] * g[siz[x] - i][siz[to] - j];
        ...
        else
        {
            ...
            {
                for (unsigned int j = 1; j <= siz[to]; ++j)
                    f[x][i + j] += f[x][i] * sum[to][j] * g[i - 1][j] * g[siz[x] - i][siz[to] - j];
                f[x][i] = 0u;
            }
        }
        siz[x] += siz[to];
...
int main()
{
    ...
    for (unsigned int i = 0; i < t; ++i)
    {
        ...
        fill(reinterpret_cast<Number*>(f), reinterpret_cast<Number*>(f) + sizeof(f) / sizeof(Number), 0u);
    }
    return 0;
}