...
void STree::add(const long long delt) { push(delt); }
...
inline void addEdge(const unsigned int x1, const unsigned int y1,
...
long long solve(const unsigned int n)
{
  for (unsigned int i = 1; i < cnt; ++i) {
    ...
    if (cp->x != seq[i - 1]->x) root->add(cp->x - seq[i - 1]->x);
