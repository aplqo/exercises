...
unsigned int findSubtree(const unsigned int x, const unsigned int rt)
{
  ...
  while (l != r) {
    ...
    if (dfn[x] > finish[mid]) l = m + 1;
    if (finish[x] < dfn[mid]) r = m - 1;
    if (x == mid || (dfn[mid] < dfn[x] && finish[x] < finish[mid])) return mid;
  }
  return chl[rt][l];
}
...
{
  if (!isChild(a, c) && !isChild(b, c)) return true;
