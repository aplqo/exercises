...
struct State
{
    ...
    inline State operator+(const State& r) const { return State { cnt + r.cnt, val + r.val }; }
    inline State operator-(const State r) const { return State { cnt - r.cnt, val - r.val }; }
    inline State operator+(const long long v) const { return State { cnt, val + v }; }
};
State stat[maxn + 10][4];

inline void addEdge(const unsigned int from, const unsigned int to, const long long val)
{
    static Edge* cur = ed;
