...
inline void reverseCopy(Number dest[], const ConstArrayPtr src, const unsigned int lgn)
{
...
template <class Func>
inline void convolution(const ConstArrayPtr& a, const ConstArrayPtr& b, ArrayPtr dest, Func pred)
{
...
void inverse(const ConstArrayPtr src, ArrayPtr dest)
{
    ...
    for (; (1u << i) <= src.size; ++i)
        convolution(
            ConstArrayPtr { src.ptr, 1u << i },
            ConstArrayPtr { pDest, 1u << (i - 1) },
            ArrayPtr { pDest, 1u << (i + 1) },
            pred);
    for (; i <= lgn; ++i)
        convolution(
            src,
...
Number recursion(const ConstArrayPtr pol, const Number iv[], unsigned long long n)
{
