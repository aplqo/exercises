...
void convolution(const ConstArrayPtr& a, const ConstArrayPtr& b, Number dest[], unsigned int lgn, Func pred)
{
...
inline void inverse(const ConstArrayPtr a, ArrayPtr& dest)
{
    ...
    convolution<false, false, true>(
        a,
        ConstArrayPtr { dest.ptr, 1u << (lgn - 1) },
        dest.ptr,
        lgn + 1,
        pred);
    dest.size = a.size;
...
inline void ntt(const ConstArrayPtr& a, const ConstArrayPtr& b, ArrayPtr& dest)
{
    ...
    convolution<op1, op2, op3>(a, b, dest.ptr, Log2(dest.size), multiplies<Number>());
}
...
inline ConstArrayPtr transMultiply(const Number vx[], const ConstArrayPtr& y, const unsigned int lgn)
{
...
void productX(const unsigned int cid, const unsigned int l, const unsigned int lg, const Number x[])
{
    if (!lg)
    {
        cur[0] = 1;
...
void evalPolynomial(const unsigned int cid, const unsigned int l, const unsigned int lg, Number dest[])
{
    if (!lg)
    {
        dest[l] = t[cid].ptr[0];
...
void eval(const Number pol[], const Number pos[], Number dest[], const unsigned int lgn)
{
