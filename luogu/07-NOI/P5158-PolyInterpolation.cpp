...
inline void reverseCopy(Number dest[], ConstArrayPtr src,
...
void convolution(const ConstArrayPtr& a, const ConstArrayPtr& b, ArrayPtr dest,
                 const Func pred)
{
...
inline void updateInverse(const ConstArrayPtr pol, Number dest[],
                          const unsigned int n)
{
  convolution(pol, ConstArrayPtr{dest, n >> 1}, ArrayPtr{dest, n << 1},
...
void productX(const unsigned int cid, const unsigned int l,
              const unsigned int r, const Number v[])
{
...
ConstArrayPtr transMultiply(const Number vx[], const ConstArrayPtr& y,
                            const unsigned int lgn)
{
...
void evalPoly(const unsigned int cid, const unsigned int l,
              const unsigned int r, Number dest[])
{
  if (r - l == 1) {
    dest[l] = t[cid].ptr[0];
    return;
  ...
  const unsigned int left = cid << 1, right = left + 1, mid = (l + r) >> 1;
  {
    ...
    const unsigned int lgn =
        lg[t[cid].size + max(cprod[right].size, cprod[left].size) - 1];
...
void evaluation(const ConstArrayPtr& pol, const ConstArrayPtr& pos,
                Number dest[])
{
...
void conquer(const unsigned int cid, const unsigned int l, const unsigned int r,
             const Number v[])
{
  if (r - l == 1) {
    cur[InterpMem][0] = v[l];
  ...
  conquer<false>(right, mid, r, v);
  {
    static Number tmpa[maxn + 10], tmpb[maxn + 10], tmpc[maxn + 10],
        tmpd[maxn + 10];
    const unsigned int actualLen = max(f[left].size + cprod[right].size,
                                       f[right].size + cprod[left].size) -
                                   1;
