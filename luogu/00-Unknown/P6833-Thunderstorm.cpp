...
struct Point {
  template <class T>
  inline T& operator[](Grid<T> dat) const
  {
    return dat[x][y];
  }
  template <class T>
  inline T operator[](const Grid<T> dat) const
  {
    return dat[x][y];
  }
...
void dijkstra(const Point src, Grid<unsigned long long> res)
{
  ...
  fill(res[0],
       res[0] + sizeof(Grid<unsigned long long>) / sizeof(unsigned long long),
       inf);
  ...
  while (!q.empty()) {
    ...
    for (unsigned int i = 0; i < 4; ++i) {
      const unsigned int nx = int(cur.second.x) + dx[i],
                         ny = int(cur.second.y) + dy[i];
