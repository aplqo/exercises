...
inline unsigned long long inverse(const unsigned long long a, const unsigned long long e)
{
...
{
    unsigned long long ret = 0;
    const unsigned long long prd = std::accumulate(b, b + k, 1ull, std::multiplies<unsigned long long>());
    for (unsigned int i = 0; i < k; ++i)
    {
        const unsigned long long v = multiply(inverse(prd / b[i], b[i]), prd / b[i], prd);
