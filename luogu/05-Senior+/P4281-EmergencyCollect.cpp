...
char inbuf[maxn * 30 + 1000], *inpos = inbuf, outbuf[maxm * 20 + 1000],
                              *outpos = outbuf;
...
pair<unsigned int, unsigned int> findVertex(const unsigned int x,
                                            const unsigned int y,
                                            const unsigned int z)
{
  static const auto dist = [](const unsigned int x, const unsigned int y,
...
int main()
{
  ...
  for (unsigned int i = 0; i < m; ++i) {
    ...
    const auto [c, p] = findVertex(x, y, z);
    writeUInt(p, c);
  }
