...
class STree {
 public:
  ...
  pair<unsigned int, unsigned int> find()
  {
    ...
    if (!gVal) return make_pair(rv, rv);
    if (rv - lv == 1) return gVal ? make_pair(lv, rv) : make_pair(rv, rv);
    if (lVal) return make_pair(lv, rv);
    pushdown();
    const auto l = lc->find();
    if (!empty(l) && l.second < mv) return l;
    const auto r = rc->find();
    if (empty(l) && empty(r)) return make_pair(rv, rv);
    if (empty(l) && !empty(r)) return r;
    if (r.first != l.second && !empty(l)) return l;
