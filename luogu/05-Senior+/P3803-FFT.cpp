...
{
    Complex() = default;
    Complex(const Real v, const Real m = 0)
        : mreal(v)
        , imag(m)
    {
    }
    ...
    {
        return Complex { mreal * b.mreal - imag * b.imag, imag * b.mreal + mreal * b.imag };
    }
    inline void operator*=(const Complex b) { *this = *this * b; }
    inline Complex operator+(const Complex b) const { return Complex { mreal + b.mreal, imag + b.imag }; }
    inline Complex operator-(const Complex b) const { return Complex { mreal - b.mreal, imag - b.imag }; }
    inline Complex operator/(const Real v) const { return Complex { mreal / v, imag / v }; }
