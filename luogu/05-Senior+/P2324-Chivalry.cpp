...
struct State {
  ...
  inline constexpr bool operator==(const State& r) const
  {
    return stat == r.stat && pos == r.pos;
  }

  unsigned long long stat;
  unsigned int pos;
};
constexpr State finalState{0b00000'10000'11000'11110'11111, 12};
  ...
  for (int i = 0; i < 5; ++i)
    for (int j = 0; j < 5; ++j) {
      ...
      for (unsigned int k = 0; k < 8; ++k) {
        ...
        if (0 <= nx && nx < 5 && 0 <= ny && ny < 5) *(cnt[cid]++) = id(nx, ny);
      }
    }
}

inline unsigned int State::estimate() const
{
  return __builtin_popcount(stat ^ finalState.stat) + (pos != finalState.pos);
}
...
unsigned int idaStar(const State ini)
{
  for (unsigned int i = 1; i < 16; i *= 2)
    if (const unsigned int t = dfs(ini, i); t != inf) return t;
  return dfs(ini, 15);
}
void solve()
{
