...
#undef decl
    ...
    inline constexpr Number operator-() const { return Number { val ? mod - val : 0 }; }
...
void inverse(const Number a[], Number dest[], const unsigned int lgn)
{
    dest[0] = inverse(a[0]);
    for (unsigned int i = 1; i <= lgn; ++i)
        convolution<0, 1>(a, dest, dest, i, [](Number a, Number b) { return b * (Number { 2 } - a * b); });
}
...
Number solve(const unsigned int lgn, const unsigned int n)
{
    ...
    a[0].val = a[1].val = 1;
    for (unsigned int i = 2; i <= n; ++i)
        a[i] = quickPow(Number { 2 }, static_cast<unsigned long long>(i) * (i - 1) >> 1) * ifac[i];
    ln(a, dest, lgn);
    return dest[n] * fac[n];
}
