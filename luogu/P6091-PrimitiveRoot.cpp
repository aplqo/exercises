...
unsigned int phiFactor[maxn + 10], *endF = phiFactor, primitiveRoot[maxn + 10],
...
{
  if (std::gcd(a, mod) != 1) return false;
  for (const unsigned int* i = phiFactor; i < endF; ++i)
    if (quickPow(a, phi / *i, mod) == 1) return false;
  return true;
}
...
{
  endRoot = primitiveRoot;
  if (!existPrimitiveRoot(x)) return;
  ...
  for (unsigned int i = 2; i < phi; ++i) {
    ...
    if (std::gcd(i, phi) == 1) *(endRoot++) = acc;
  }
}
