...
enum Operate { Max, HistoryMax, Add, Assign };
class SegmentTree {
 public:
  void build(const unsigned int l, const unsigned int r, const int val[],
             SegmentTree*(&cur));
  template <Operate op>
...
int SegmentTree::query(const unsigned int l, const unsigned int r)
{
  ...
  if (l == lv && r == rv) return op == HistoryMax ? history.max : current.max;
  pushdown();
  if (r <= mv)
...
template <Operate op>
void SegmentTree::modify(const unsigned int l, const unsigned int r,
                         const int delt)
{
