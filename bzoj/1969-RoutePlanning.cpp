...
unsigned int SegmentTree::count(const unsigned int l,
                                const unsigned int r) const
{
  if ((l == lv && r == rv) || !val) return val;
  if (r <= mv)
    return lc->count(l, r);
  else if (l >= mv)
    return rc->count(l, r);
...
void dfsChain(const unsigned int x, const unsigned int tp)
{
  ...
  for (const TreeEdge* i = head[x]; i; i = i->pre)
    if (i->to != son[x]) dfsChain(i->to, i->to);
}
...
void clearPath(unsigned int u, unsigned int v)
{
  ...
  if (u != v) root->clear(dfn[u] + 1, dfn[v] + 1);
}
unsigned int countPath(unsigned int u, unsigned int v)
{
  ...
  if (dep[u] > dep[v]) swap(u, v);
  if (u != v) ret += root->count(dfn[u] + 1, dfn[v] + 1);
  return ret;
}
}  // namespace Chain
