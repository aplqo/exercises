...
void createChain(ofstream& f, const unsigned int n, const unsigned int len, const unsigned int mv)
{
    uniform_int_distribution<unsigned int> dis(0, mv);
    for (unsigned int i = len + 1; i > 1; --i)
        f << shf[i - 1] << " " << shf[i] << " " << dis(default_rnd) << endl;
}
void createTree(ofstream& f, const unsigned int n, const unsigned int beg, const unsigned int mv)
{
    uniform_int_distribution<unsigned int> dis(0, mv);
    for (unsigned int i = beg; i <= n; ++i)
        f << shf[GetRandom(1u, i - 1)] << " " << shf[i] << " " << dis(default_rnd) << endl;
}
