...
pair<unsigned int, unsigned long long> dfs(const unsigned int x)
{
    ...
    const auto l = dfs(nod[x].l);
    const auto r = dfs(nod[x].r);
    const unsigned long long ra = 1ull * l.first * r.first + l.first + r.first + 1;
    return make_pair(r.first + l.first + 1, 2 * ra * hight[x] + l.second + r.second);
}
int main()
{
#ifndef APTEST
    ...
    buildSA(str, l);
    unsigned long long ans = 1ull * (l - 1) * (1 + l) * l / 2 - dfs(build(l)).second;
    cout << ans << endl;
    return 0;
}