...
struct Line {
  ...
  inline pair<long long, long long> equalRange(const long long l,
...
inline long long Line::solve(const Line& r) const
{
  ...
  if (cv < 0 || (*this)(cv) != r(cv)) fail();
  return cv;
}
inline pair<long long, long long> Line::equalRange(const long long l,
                                                   const long long r) const
{
  const auto sol = [this](const long long p) {
    return static_cast<double>(p - b) / k;
  };
...
bool dfsTrySolve(const unsigned int x, const unsigned int fa,
                 const unsigned int root)
{
  ...
  for (const Edge* i = head[x]; i; i = i->pre) {
    ...
    equ[to] = eto;
    if (dfsTrySolve(to, x, root)) return true;
  }
  return false;
}
void dfsSolve(const unsigned int x, const unsigned int fa,
              const unsigned int root)
{
  ...
  sum[root] += equ[x];
  if (equ[x].k != 0) {
    const auto v = equ[x].equalRange(0, p[x]);
    mn[root] = max(mn[root], v.first);
    mx[root] = min(mx[root], v.second);
  }
...
int main()
{
  ...
  for (unsigned int i = 0; i < m; ++i) {
    ...
    long long w;
    cin >> u >> v >> w;
    if ((u != v && p[u] + p[v] < w) || (u == v && p[u] < w)) fail();
