...
pair<unsigned int, unsigned int> findDiameter()
{
  ...
  const auto pat = dfsPathMark(pos, 0);
  markDiameter(pos, pat.first);
  return make_pair(pos, pat.second);
}
bool getDisSub(const unsigned int endp, const unsigned long long dia)
{
  for (unsigned int i = endp; i; i = nxt[i]) {
    const auto pat = dfsPath(i, 0);
    if (dfsPath(pat.second, 0).second == dia) return false;
    disSub[i] = pat.first;
  }
  return true;
}
...
int main()
{
  ...
  const auto ep = findDiameter();
  cout << disR[ep.first] << "\n";
  cout << (getDisSub(ep.first, disR[ep.second]) ? coutEdge(ep.first, ep.second)
                                                : 0)
       << "\n";
  return 0;
}