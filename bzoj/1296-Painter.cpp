        ...
        for (unsigned int t = 1; t <= i; ++t)
        {
            ...
            for (unsigned int k = 1; k < i; ++k)
            {
                if (board[p][k] == board[p][i])
                    f[i][t][1] = max(f[i][t][1], f[k][t][1] + cnt[i][board[p][i]] - cnt[k][board[p][i]]);
                else
                    f[i][t][1] = max(f[i][t][1], f[k][t - 1][1] + cnt[i][board[p][i]] - cnt[k][board[p][i]]);
            }
        }
    for (unsigned int i = 1; i <= m; ++i)
...
unsigned int dp(const unsigned int n, const unsigned int m, const unsigned int t)
{
    for (unsigned int i = 1; i <= n; ++i)
