...
int main()
{
    ...
    read(n);
    make();
    unsigned int cnt = count_if(a + 1, cur, [](const node& i) { return i.val > 0; });
    num_t ans = accumulate(a + 1, cur, num_t(0), [](num_t v, const node& i) { return i.val > 0 ? v + i.val : v; });
