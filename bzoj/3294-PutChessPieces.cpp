...
Number C[maxs + 10][maxs + 10], use[maxs + 10][maxn + 10][maxn + 10], col[maxc + 10][maxn + 10][maxn + 10];
...
static void dpUse(const unsigned int n, const unsigned int m, const unsigned int k)
{
    for (unsigned int i = 1; i <= n; ++i)
        for (unsigned int j = 1; j <= m; ++j)
        {
            ...
            for (unsigned int r = 1; r <= i; ++r)
                for (unsigned int c = 1; c <= j; ++c)
                    if (r < i || c < j)
                        use[k][i][j] -= use[k][r][c] * C[i][r] * C[j][c];
        }
}
static void dpColor(const unsigned int n, const unsigned int m, const unsigned int colCnt)
{
    col[0][0][0] = 1;
            ...
            for (unsigned int j = 1; j <= m; ++j)
            {
                for (unsigned int l = 0; l < i; ++l)
                    for (unsigned int r = 0; r < j; ++r)
                        if ((i - l) * (j - r) >= cnt[k])
                            col[k][i][j] += col[k - 1][l][r] * use[cnt[k]][i - l][j - r] * C[n - l][i - l] * C[m - r][j - r];
            }
}
