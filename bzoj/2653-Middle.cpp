...
public:
    ...
    inline int query(const unsigned int l1, const unsigned int l2, const unsigned int r1, const unsigned int r2) const
    {
        return queryImpl(l1, l2 + 1).post + (l2 + 1 != r1 ? queryImpl(l2 + 1, r1).sum : 0) + queryImpl(r1, r2 + 1).pre;
    }
...
private:
    ...
    {
        value_type() = default;
        value_type(int pre, int post, int sum)
            : pre(pre)
            , post(post)
            , sum(sum) {};
        value_type(int a)
        ...
        {
            return value_type(max(pre, sum + r.pre), max(r.post, r.sum + post), sum + r.sum);
        }
...
static void build(const unsigned int n)
{
    ...
    iota(pos, pos + 1 + n, 0);
    sort(pos, pos + n, [](const unsigned int a, const unsigned int b) { return val[a] < val[b]; });
    {
...
long long query(const unsigned int n, const unsigned int l1, const unsigned int l2, const unsigned int r1, const unsigned int r2)
{
    int l = 0, r = n - 1;
    while (l <= r)
    {
...
int main()
{
    ...
    for (unsigned int i = 0; i < q; ++i)
    {
        ...
        cin >> q[0] >> q[1] >> q[2] >> q[3];
#ifndef NoEncode
        transform(q, q + 4, q, [lst, n](long long v) -> unsigned int { return (v + lst) % n; });
#endif
        sort(q, q + 4);
