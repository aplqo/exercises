...
class Splay {
 ...
 private:
  ...
  struct Value {
    ...
    inline unsigned int answer()
    {
      return (-pref[Min] + 1) / 2 + (suff[Max] + 1) / 2;
    }
  };
  struct node {
    void pushdown();
...
{
  if (!tagAss && !tagInv && !tagSwp) return;
  for (Splay::node* i : chl) {
    if (!i) continue;
    if (tagAss) i->pushAss(tagAss);
    if (tagInv) i->pushInv();
    if (tagSwp) i->pushSwp();
  }
  tagInv = tagSwp = false;
...
void Splay::node::pushup()
{
  ...
  const Value &lv = chl[0] ? chl[0]->info : nil,
              &rv = chl[1] ? chl[1]->info : nil;
  info.sum = val;
  siz = 1;
