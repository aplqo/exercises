...
pair<unsigned long long, unsigned long long> SegmentTree::merge(
    SegmentTree* const old)
{
  static const auto may = [](const SegmentTree* ptr) -> unsigned long long {
  ...
  if (!old || rv - lv == 1) return make_pair(0ull, 0ull);
  const auto p =
      make_pair(may(this->rc) * may(old->lc), may(this->lc) * may(old->rc));
