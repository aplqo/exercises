...
void createQuery(ofstream& in, const unsigned int n, const unsigned int m, const unsigned long long mv)
{
    uniform_int_distribution<unsigned int> dis(1, 2), disp(1, n);
    uniform_int_distribution<unsigned long long> disv(1, mv);
    for (unsigned int i = 0; i < m; ++i)
    {
        unsigned int op = dis(default_rnd), p1 = disp(default_rnd), p2 = disp(default_rnd);
        if (p1 > p2)
            swap(p1, p2);
