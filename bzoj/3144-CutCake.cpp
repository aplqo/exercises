...
const unsigned int maxv = maxn * maxn * maxn + 2, maxe = (maxn * maxn * maxn * 5 * 2 + maxn * maxn) * 2;
...
pair<unsigned int, unsigned int> buildGraph(const int p, const int q, const int r, const int d)
{
    ...
    for (unsigned int i = 0; i < r; ++i)
        for (unsigned int j = 0; j < p; ++j)
            for (unsigned int k = 0; k < q; ++k)
                cin >> mat[j][k][i];
    const auto st = buildGraph(p, q, r, d);
    cout << isap(st.first, st.second, st.second) << endl;
    return 0;
}