...
{
  if (vis[st.pos].test(st.step)) return;
...
unsigned int bfs(const unsigned int n)
{
  ...
  while (!q.empty()) {
    ...
    const auto val = q.front().second;
    if (cur.pos == b[1]) return val;
    q.pop_front();
    if (!doges[cur.pos].empty()) {
      for (unsigned int i : doges[cur.pos])
        if (i != cur.step) update<0>(q, State{cur.pos, i}, val);
      doges[cur.pos].clear();
    }
    if (cur.pos >= cur.step)
