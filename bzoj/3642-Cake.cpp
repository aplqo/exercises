...
typedef int Rank;
constexpr Rank inf = numeric_limits<Rank>::max(), ninf = numeric_limits<Rank>::min();

class SegmentTree
{
public:
    void build(const unsigned int l, const unsigned int r, SegmentTree*& cur, const Rank dat[]);
...
void SegmentTree::build(const unsigned int l, const unsigned int r, SegmentTree*& cur, const Rank dat[])
{
    lv = l;
    rv = r;
    mv = (l + r) >> 1;
...
static void init(const unsigned int n, const unsigned int a)
{
    ...
    sort(tops.begin() + 1, tops.end(), [](unsigned int a, unsigned int b) { return d[a] > d[b]; });
...
void enhance(const unsigned int pos, const unsigned int e, const unsigned int a)
{
    ...
    {
        const auto it = find(tops.begin(), tops.end(), pos);
        if (it != tops.end())
            tops.erase(it);
    }
    ...
    {
        if (*it != a)
            roots[*it > a]->assign(distance(*it, a), ++lst);
        d[*it] = lst;
    }
}
