...
template <bool dir>
void conquer(const unsigned int pl, const unsigned int pr,
             const unsigned int vl, const unsigned int vr)
{
  if (pl > pr) return;
  static
      typename conditional<dir, plus<unsigned int>, minus<unsigned int>>::type
          nxt;
  static
      typename conditional<dir, less<unsigned int>, greater<unsigned int>>::type
          cmp;
  const unsigned int mid = (pl + pr) >> 1;
  unsigned int pos = mid;
  mx[dir][mid] = a[mid];
  for (unsigned int i = min(vl, vr, cmp); vl <= i && i <= vr && cmp(i, mid);
       i = nxt(i, 1)) {
