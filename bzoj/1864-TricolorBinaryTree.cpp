...
unsigned int dp(const unsigned int pos)
{
  ...
  if (str[pos] == '1') {
    ...
    ret += dp(ch2);
    for (unsigned int i = 0; i < 3; ++i) {
      mx[pos][i] = max(mx[ch1][(i + 1) % 3] + mx[ch2][(i + 2) % 3],
                       mx[ch2][(i + 1) % 3] + mx[ch1][(i + 2) % 3]);
      mn[pos][i] = min(mn[ch1][(i + 1) % 3] + mn[ch2][(i + 2) % 3],
                       mn[ch2][(i + 1) % 3] + mn[ch1][(i + 2) % 3]);
    }
  }
  ++mx[pos][0];
  ++mn[pos][0];
...
int main()
{
  ...
  cin >> str;
  dp(0);
  cout << *max_element(mx[0], mx[0] + 3) << " "
       << *min_element(mn[0], mn[0] + 3) << "\n";
  return 0;
}