...
namespace Flow
{
    ...
    void bfs(const unsigned int t)
    {
        ...
        while (!q.empty())
        {
            ...
            for (const Edge* i = head[x]; i; i = i->pre)
                if (enable[i->to] == tok && vis[i->to] != sta && i->rev->cap > i->rev->flw)
                {
                    dep[i->to] = dep[x] + 1;
                    vis[i->to] = sta;
                    q.push(i->to);
    ...
    template <unsigned int tok>
    unsigned int isap(const unsigned int nb, const unsigned int s, const unsigned int t)
    {
...
unsigned int clique(const unsigned int na, const unsigned int nb)
{
    ...
    if (vertexb[0].size() > vertexb[1].size())
        vertexb[0].swap(vertexb[1]);
    ...
    for (unsigned int a : vertexa[0])
        for (unsigned int b : vertexa[1])
        {
            ...
            if (c + 2 > ret)
                ret = max(ret, c + 2 - Flow::isap<2>(nb + 2, 0, nb + 1));
    ...
    for (unsigned int i = 1; i <= na; ++i)
    {
        ...
        if (c + 1 > ret)
            ret = max(ret, c - Flow::isap<1>(nb + 2, 0, nb + 1) + 1);
        memset(enable, 0, sizeof(enable));
    }
    return ret;
}
...
unsigned int solve()
{
    ...
    for (unsigned int v1 : vertexb[0])
    {
        ...
        for (unsigned int v2 : vertexb[1])
            if (!(popcnt(vb[v1] | vb[v2]) & 0x01))
                Flow::addEdge(v1, v2, inf);
    }
    for (unsigned int v2 : vertexb[1])
