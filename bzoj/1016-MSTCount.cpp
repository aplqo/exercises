...
#define impl(x) \
    inline Number operator x(const Number l, const Number r) { return (l.val + mod) x r.val; }
impl(+);
impl(-);
impl(*);
#undef impl
...
Number kruskal(const unsigned int n, const unsigned int m)
{
    ...
    {
        if (i->vis == Edge::Visited)
            continue;
        const auto t = count(i, ed + m);
        ret *= t.first;
        c += t.second;
    }
    if (c < n)
