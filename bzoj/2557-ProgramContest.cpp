...
constexpr unsigned int maxv = maxn + maxm + 2,
                       maxe = (maxn + maxm + maxm * maxn) * 2;
...
unsigned int isap(const unsigned int n, const unsigned int s,
  ...
  while (dep[s] < n) {
    ...
    if (!adv) {
      ...
      for (const Edge* i = head[x]; i; i = i->pre)
        if (i->cap > i->flw) miv = min(miv, dep[i->to]);
      --cnt[dep[x]];
      if (!cnt[dep[x]]) break;
      dep[x] = miv + 1;
      ++cnt[dep[x]];
...
pair<unsigned int, unsigned long long> solve(const unsigned int n,
                                             const unsigned int m,
                                             const unsigned int r,
                                             const unsigned int tim)
{
