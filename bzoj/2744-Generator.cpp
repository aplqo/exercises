...
void createGraph(ofstream& in, const unsigned int m, const unsigned int na, const unsigned int nb)
{
...
void createSeq(ofstream& in, const unsigned int len)
{
    ...
    generate_n(ostream_iterator<unsigned int>(in, " "), len, bind(dis, ref(default_rnd)));
    in.put('\n');
}
...
int main(int, char* argv[])
{
    ...
    uniform_int_distribution<unsigned int> disa(1, MayBe(atoi(argv[2]), maxa)), disb(1, MayBe(atoi(argv[3]), maxb));
    ofstream in(argv[5]);
    in << t << "\n";
    for (unsigned int i = 0; i < t; ++i)
    {
        const unsigned int a = disa(default_rnd), b = disb(default_rnd);
        const unsigned int m = GetRandom(0u, min(MayBe<unsigned int>(atoi(argv[4]), a * b), a * b));
        in << a << " " << b << " " << m << "\n";
        createSeq(in, a);
        createSeq(in, b);
