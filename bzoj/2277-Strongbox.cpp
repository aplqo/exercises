...
void markInvalid(const unsigned int ptr, const unsigned int k, const unsigned long long cand)
{
    sort(fac, fac + ptr);
    for (const unsigned long long* i = a; i < a + k - 1; ++i)
        invalid[lower_bound(fac, fac + ptr, __gcd(cand, *i)) - fac] = true;
}
unsigned long long count(const unsigned long long n, const unsigned int k)
{
    const unsigned long long cand = __gcd(n, a[k - 1]), ptr = factor(cand);
