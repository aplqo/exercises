...
public:
    void build();
    inline SegmentTree* init(const unsigned int l, const unsigned int r, const unsigned int sta);
...
inline SegmentTree* SegmentTree::init(const unsigned int l, const unsigned int r, const unsigned int sta)
{
    lv = l;
    rv = r;
    this->sta = sta;
...
void SegmentTree::assign(const SegmentTree* old, const unsigned int pos, const Node& val)
{
    if (rv - lv == 1)
    {
        this->dat.val = val;
...
const SegmentTree* merge(const SegmentTree* const root, const unsigned int n, const unsigned int x, const unsigned int y)
{
...
int main()
{
    ...
    for (unsigned int i = 1; i <= m; ++i)
    {
        ...
        case 1:
            cin >> a >> b;
            root[i] = merge(root[i - 1], n, a ^ lst, b ^ lst);
            break;
        case 2:
            cin >> a;
            root[i] = root[a ^ lst];
        ...
        case 3:
            root[i] = root[i - 1];
            cin >> a >> b;
            cout << (lst = (find(root[i], a ^ lst).fa == find(root[i], b ^ lst).fa)) << "\n";
            break;
        }
    }
    return 0;
