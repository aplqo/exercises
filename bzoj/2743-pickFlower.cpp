...
char inbuf[maxn * 10 + maxm * 10 + 1000],
    *inpos = inbuf, outbuf[maxm + maxm * 10 + 1000], *pos = outbuf;
...
inline void write(unsigned int v) { pos += sprintf(pos, "%u\n", v); }
...
inline void increase(const unsigned int l, const unsigned int r,
                     const unsigned int n)  // [l,r) +=1
{
  modify(l, n, 1);
  modify(r, n, -1);
}
...
void solveQuery(const unsigned int n, const unsigned int q)
{
  ...
  for (unsigned int i = 1; i <= n; ++i) {
    ...
    if (lst1) ftree::increase(lst2 + 1, lst1 + 1, n + 1);
    for (; p < q && seq[p]->r == i; ++p)
