...
public:
    ...
    {
        auto* p = lower_bound<const pair<unsigned long long, unsigned int>*>(xval, xen, make_pair(xv, 0u));
        return p == xen || p->first != xv ? r : p->second;
    }
    unsigned int findPos(const unsigned long long ex, unsigned long long gs, unsigned long long xs) const
    {
        for (unsigned int i = l; i < r; ++i)
        {
            gs = gcd(gs, a[i]);
...
{
    {
        const unsigned int p = blk[0].findPos(x, a[0], 0);
        if (p != blk[0].r)
            return p;
    }
    ...
    for (unsigned int i = 1; i <= (n - 1) / bsize; ++i)
    {
        ...
        {
            const auto p = blk[i].findPos(x, gs, xs);
            if (p != blk[i].r)
                return p;
        }
        gs = ngs;
