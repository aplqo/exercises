...
void dfsDCTree(const unsigned int x)
{
  ...
  for (auto i = orig.head[x]; i; i = i->pre) {
    const unsigned int to = i->to;
    if (dcvis[to]) continue;
    dep[to] = dfsDepth(to, x);
    dis[to] = i->val;
    subs.push_back(to);
  }
  sort(subs.val, subs.cur,
       [](unsigned int a, unsigned int b) { return dep[a] > dep[b]; });
  for (auto i : subs)
    nxt.addEdge(x, i, dis[i]);

  subs.clear();
  for (auto i = orig.head[x]; i; i = i->pre) {
    if (dcvis[i->to]) continue;
  ...
  *(r++) = x;
  ...
  while (l < r) {
    ...
    for (auto i = orig.head[c]; i; i = i->pre) {
      const unsigned int to = i->to;
      if (dcvis[to] || vis[to] == sta) continue;
      vis[to] = sta;
      dep[to] = dep[c] + 1;
