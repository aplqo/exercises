...
class STree {
 public:
  ...
  [[nodiscard]] unsigned int query(const unsigned int l,
...
void STree::replace(const unsigned int l, const unsigned int r,
                    const unsigned int v)
{
  if (l == lv && r == rv) {
    push(v);
...
[[nodiscard]] unsigned int STree::query(const unsigned int l,
                                        const unsigned int r) const
{
  if (laz || (l == lv && r == rv)) return val;
  if (r <= mv)
    return lc->query(l, r);
  else if (l >= mv)
    return rc->query(l, r);
...
void STree::push(const unsigned int l) { val = laz = l; }
void STree::pushdown()
{
  if (!laz) return;
  lc->push(laz);
...
inline unsigned int queryExclude(const unsigned int x)
{
  ...
  if (dfn[x] != 1) ret = root->query(1, dfn[x]);
  if (dfn[x] + siz[x] < n + 1)
