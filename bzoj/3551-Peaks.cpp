...
class STree {
 public:
  ...
  void increase(const STree* old, const unsigned int lv, const unsigned int rv,
  ...
  unsigned int kth(const STree* old, const unsigned int lv,
                   const unsigned int rv, const unsigned int k) const
  {
    if (rv - lv == 1) return lv;
    const unsigned int mv = (lv + rv) >> 1;
    const unsigned int lsum = this->lc->val - old->lc->val;
    return k <= lsum ? lc->kth(old->lc, lv, mv, k)
                     : rc->kth(old->rc, mv, rv, k - lsum);
  }
...
unsigned int fa[maxv + 1][maxl + 1], dfn[maxn + 1], dep[maxv + 1],
    inv[maxn + 1];
...
void dfs(const unsigned int x, const unsigned int f)
{
  ...
  for (const VirtEdge* i = head[x]; i; i = i->pre)
    if (i->to != f) {
      dfs(i->to, x);
      siz[x] += siz[i->to];
      if (first) leftPos[x] = leftPos[i->to];
      first = false;
    }
}
...
unsigned int query(const unsigned int v, const unsigned int x,
                   const unsigned int k)
{
  const unsigned int p = findRoot(v, x);
  if (siz[p] < k) return UINT_MAX;
  return orig[root[dfn[leftPos[p]] + siz[p] - 1]->kth(root[dfn[leftPos[p]] - 1],
                                                      0, n, siz[p] + 1 - k)];
}
...
int main(int, char* argv[])
{
  ...
  for (unsigned int i = 0; i < q; ++i) {
    const auto v = readInt() ^ lst;
    const auto x = readInt() ^ lst;
    const auto k = readInt() ^ lst;
