...
real dfs(const unsigned int p, int v, const unsigned int cnt = 0)
{
    ...
    if (p >= pos && v >= 0 && !cnt)
        return 1;
    if (p >= pos && v + sum[n] - sum[p - 1] < 0 || cnt > n - p + 1)
        return 0;
    ...
    return mem[cv] = dfs(p + 1, v + cha[p].a, cnt >= 1 ? cnt - 1 : 0) * cha[p].p + dfs(p + 1, v, cnt) * (1 - cha[p].p);
}
...
int main()
{
    ...
    for (unsigned int i = 1; i <= n; ++i)
        scanf("%d", &cha[i].a);
    sort(cha + 1, cha + n + 1, [](const challange& a, const challange& b) { return a.a != b.a ? a.a < b.a : a.p > b.p; });
    pos = find_if_not(cha + 1, cha + 1 + n, [](const challange& i) { return i.a == -1; }) - cha;
