...
void dfsCentroid(const unsigned int x, const unsigned int f,
...
pair<unsigned long long, unsigned int> dfsChain(const unsigned int x,
                                                const unsigned int f,
                                                const unsigned int dep)
{
  ...
  for (const Edge* i = head[x]; i; i = i->pre) {
    const unsigned int to = i->to;
    if (to != f) {
      const auto v = dfsChain<false>(to, x, dep + 1);
      siz[x] += siz[to];
      ret.first += v.first;
  ...
  if (isRoot) {
    if (ms * 2 == siz[x]) ret.second = mdep[son];
  }
  else
    mdep[x] = ret.second;
  return ret;
...
int main()
{
  ...
  for (unsigned int i = 1; i <= n; ++i)
    if (i != centroid[0] && i != *(ccur - 1))
      ...
      const auto v = dfsChain<true>(i, 0, 0);
      cout << v.first * 2 - v.second << "\n";
    }
  return 0;
}