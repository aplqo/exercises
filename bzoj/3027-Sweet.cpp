...
Number count(map<unsigned int, Number>&& mp, const unsigned int n, const unsigned int a, const unsigned int b)
{
    Number ret = 0;
    for (auto i : mp)
        if (i.first <= b)
            ret += i.second * (C(n + b - i.first, b - i.first) - (a >= i.first + 1 ? C(n + a - i.first - 1, a - i.first - 1) : 0));
    return ret;
}
