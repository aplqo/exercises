...
inline Range maxRange(const unsigned int lpos, const unsigned int rl, const unsigned int rr)
{
    const MaxVal v = st.max(rl, rr);
    return Range { v.val - sum[lpos - 1], lpos, v.pos, rl, rr };
}
long long solve(const unsigned int l, const unsigned int r, const unsigned int n, const unsigned int k)
{
    ...
    for (unsigned int i = 0; i < k; ++i)
    {
        ...
        ret += t.val;
        if (t.maxPos > t.l)
            q.push(maxRange(t.leftPos, t.l, t.maxPos - 1));
        if (t.maxPos < t.r)
            q.push(maxRange(t.leftPos, t.maxPos + 1, t.r));
    }
    return ret;
}
int main()
