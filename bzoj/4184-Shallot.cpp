...
{
    return accumulate(vec, vec + maxl + 1, 0u, [](unsigned int a, unsigned int b) { return a ^ b; });
}
...
void STree::add(const unsigned int l, const unsigned int r, const unsigned int v)
{
    if (l == lv && r == rv)
    {
        val.push_front(v);
