...
edge* addEdge(const unsigned int from, const unsigned int to, const unsigned int val)
{
    static edge* cur = ed;
    cur->to = to;
    cur->from = from;
...
{
    if (x == t)
        return make_pair(true, UINT_MAX);
    for (const edge* i = head[x]; i; i = i->pre)
    {
        if (i->to == f || i->enable < sta)
            continue;
        const auto rv = findMin(i->to, x);
        if (rv.first)
            return make_pair(true, min(rv.second, i->val));
    }
...
bool kruskal(const unsigned int n, const unsigned int m, const unsigned int val)
{
    dsu::init(n);
    ++sta;
    for (unsigned int i = 0; i < 2 * m && dsu::getFather(s) != dsu::getFather(t); ++i)
    {
...
inline unsigned int rat(const unsigned int n, const unsigned int m, const unsigned int mv)
{
    if (!kruskal(n, m, mv))
        return UINT_MAX;
    const auto rv = findMin(s, 0);
    return rv.first ? rv.second : UINT_MAX;
}
static void init(const unsigned int m)
{
    transform(ed, ed + m * 2, seq, [](const edge& a) { return &a; });
    sort(seq, seq + m * 2, [](const edge* a, const edge* b) { return a->val > b->val; });
}
...
int main()
{
    ...
    init(m);
    auto rv = solve(n, m);
    if (rv.first == UINT_MAX)
        cout << "IMPOSSIBLE" << endl;
    else
    {
        const unsigned int g = gcd(rv.first, rv.second);
        rv.first /= g;
        rv.second /= g;
        if (rv.second > 1)
