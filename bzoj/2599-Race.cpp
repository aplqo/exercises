...
void adEdge(const unsigned int from, const unsigned int to, const unsigned int val)
{
    static edge* cur = ed;
    cur->to = to;
    cur->val = val;
...
{
    if (dep[x] > ans || dis[x] > k)
        return;
    {
        auto it = pat.find(k - dis[x]);
        if (it != pat.end())
...
void divide(const unsigned int x)
{
    ...
    for (const edge* i = head[x]; i; i = i->pre)
        if (!vis[i->to])
            divide(treeCenter(siz[i->to], i->to));
}
int main()
{
#ifndef APTEST
