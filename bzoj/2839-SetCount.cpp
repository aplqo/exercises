...
#define decl(x)                             \
  ...
  decl(-);
#undef decl
  unsigned long long toull() const { return val; }
...
#define impl(x)                                                   \
  template <unsigned long long mod>                               \
  inline Number<mod> Number<mod>::operator x(const Number<mod> r) \
  {                                                               \
    return this->val x r.val;                                     \
  }
impl(*);
impl(+);
#undef impl
...
Number<M> calc(const unsigned long long n, const unsigned long long k)
{
  ...
  for (unsigned long long i = k; i <= n; ++i) {
    const Number<M> t =
        c(i, k) * c(n, i) * quickPow<M>(2, quickPow<M - 1>(2, n - i).toull());
    if ((i - k) & 0x01)
      acc -= t;
    else
      acc += t;
