...
{
  static
      typename conditional<inc, plus<unsigned int>, minus<unsigned int>>::type
          op;
  size = op(size, 1);
  if (his.empty() || his.back().size < size) his.push_back(History{time, size});
}
...
unsigned int query(const char str[], const unsigned int v)
{
  ...
  return !ptr || ptr->his.back().size <= v
             ? 0
             : upper_bound(ptr->his.cbegin(), ptr->his.cend(),
                           History{0, v + 1})
                   ->time;
}
