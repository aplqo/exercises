...
{
    if (!s.len)
        return !s.dsum && !s.con ? 1 : 0;
    if (s.len < 2)
        return s.dsum < 10 ? s.dsum % s.mod == s.con : 0;
    if (s[vis])
        return s[mem];
    unsigned long long ret = 0;
    for (int i = 0; i < 10 && i <= s.dsum; ++i)
        ret += f(State(s.len - 1, s.dsum - i, s.mod, ((s.con - i * po[s.len - 1]) % s.mod + s.mod) % s.mod));
    s[vis] = true;
    return s[mem] = ret;
}
    ...
    for (int s = 1; s <= 9 * (l - 1) + digits[l]; ++s)
    {
        ...
        for (unsigned int i = l; i > 0 && acc <= s; --i)
        {
            for (unsigned int j = 0; j < digits[i] && acc + j <= s; ++j)
                ret += f(State(i - 1, s - acc - j, s, ((s - j * po[i - 1] - v) % s + s) % s));
            acc += digits[i];
