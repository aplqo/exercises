...
void createCase(ofstream& in, const unsigned int n, const unsigned int m,
                const unsigned int mv)
{
  in << n << " " << m << endl;
  createSeq(in, n, mv);
...
{
  Init();
  uniform_int_distribution<unsigned int> disn(1, MayBe(atoi(argv[1]), maxn)),
      dism(1, MayBe(atoi(argv[2]), maxm));
