...
unsigned int bfsExtreme(const unsigned int s)
{
  ...
  while (!q.empty()) {
    const unsigned int cur = q.front();
    q.pop();
    if (isEnd[cur]) ret = min(ret, y[cur], cmp);
...
void westExtreme(const unsigned int* beg, const unsigned int* end,
                 unsigned int res[])
{
  T<unsigned int> cmp;
...
{
  static const auto yCmp = [](unsigned int a, unsigned int b) {
    return y[a] > y[b];
  };
  discretize(n);
