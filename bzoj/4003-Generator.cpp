...
void createH(ofstream& in, const unsigned int n, const long long mnh,
             const long long mxh)
{
  uniform_int_distribution<long long> dis(mnh, mxh);
  generate_n(ostream_iterator<long long>(in, " "), n,
             [&dis]() { return dis(default_rnd); });
  in << endl;
}
void createTree(ofstream& in, const unsigned int n, const long long mnv,
                const long long mxv)
{
  uniform_int_distribution<unsigned int> dist(0, 1);
  uniform_int_distribution<long long> disav(mnv, mxv),
      dismv(max(mnv, 1ll), mxv);
...
void createKnight(ofstream& in, const unsigned int m, const unsigned int n,
                  const long long mns, const long long mxs)
{
