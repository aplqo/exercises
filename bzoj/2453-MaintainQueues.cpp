...
{
    Modify() = default;
    Modify(const unsigned int p, const unsigned int nv)
        : pos(p)
        , newv(nv) {};
    unsigned int pos;
...
inline unsigned int replace(const unsigned int p, const unsigned int nv, const unsigned int l, const unsigned int r)
{
    if (p < l || p > r)
    {
        col[p] = nv;
...
inline unsigned int apply(const unsigned int tim, const unsigned int l, const unsigned int r)
{
    Modify& m = mod[tim];
    m.oldv = col[m.pos];
    return replace(m.pos, m.newv, l, r);
}
inline unsigned int undo(const unsigned int tim, const unsigned int l, const unsigned int r)
{
