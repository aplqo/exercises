...
#include <utility>
using namespace std;
const unsigned int maxn = 10000, maxm = 10000;
...
struct Oper {
  ...
  enum { Insert, Remove, Query } typ;
} op[maxn + maxm * 2 + 1];
Oper *seq[maxn + maxm * 2 + 1], *ql[maxn + maxm * 2 + 1],
    *qr[maxn + maxm * 2 + 1];
...
static void discretize(const unsigned int cnt)
{
  ...
  for (Oper* i = op; i < op + cnt; ++i)
    if (i->typ == Oper::Insert) *(oe++) = i->dat.chg.val;
  sort(orig + 1, oe);
  oe = unique(orig + 1, oe);
  dcnt = oe - orig;
  for (Oper* i = op; i < op + cnt; ++i)
...
void binary(const Range v, const Range q)
{
  if (v.second - v.first == 1) {
    ...
    for (unsigned int i = q.first; i < q.second; ++i)
      if (seq[i]->typ == Oper::Query) seq[i]->dat.qry.ans = nv;
    return;
  }
...
int main()
{
  ...
  for (Oper* i = op; i < op + qcnt; ++i)
    if (i->typ == Oper::Query) cout << i->dat.qry.ans << endl;
  return 0;
}