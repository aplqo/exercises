...
const unsigned int maxu = maxl,
                   maxp = INT_MAX;  // max update times, max pow process
...
class QuickPow {
 public:
  void init(const unsigned long long c, const unsigned int mod,
...
void QuickPow::init(const unsigned long long c, const unsigned int mod,
                    const unsigned int siz)
{
  blk = sqrt(siz);
  this->mod = mod;
...
{
  return (block[p / blk] * static_cast<unsigned long long>(step[p % blk])) %
         mod;
}
...
void powC(const unsigned int pos)
{
  ...
  if (!exceed[pos])
    ...
    if (!old) {
      ...
      for (unsigned int i = 0; i < pcur; ++i)
        ps[i] = memp[i].pow(
            p >= phiMod[i + 1] ? p % phiMod[i + 1] + phiMod[i + 1] : p);
    }
    else {
      for (unsigned int i = 0; i < pcur; ++i)
