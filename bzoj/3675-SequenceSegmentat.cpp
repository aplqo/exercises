...
long long solve(const unsigned int n, const unsigned int k)
{
    ...
    for (unsigned int i = 1; i <= k; ++i)
    {
        ...
        {
            while (back[lst] - front[lst] >= 2 && (Point(*(front[lst] + 1), lst) - Point(*front[lst], lst)) * Vector(1, K(j)) <= 0)
                ++front[lst];
            f[j][cur] = f[*front[lst]][lst] + (a[j] - a[*front[lst]]) * a[*front[lst]];
            const Point cp(j, cur);
            while (back[cur] - front[cur] >= 2 && (cp - Point(*(back[cur] - 2), cur)) * (cp - Point(*(back[cur] - 1), cur)) >= 0)
                --back[cur];
            *(back[cur]++) = j;
        }
    }
