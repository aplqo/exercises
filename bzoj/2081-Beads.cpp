...
struct HashValue {
  inline HashValue operator+(const unsigned int r) const
  {
    return HashValue{(v1 + r) % m1, (v2 + r) % m2};
  }
  inline HashValue operator*(const unsigned int v) const
  {
    return HashValue{(v1 * v) % m1, (v2 * v) % m2};
  }
  inline HashValue operator*(const HashValue v) const
  {
    return HashValue{(v1 * v.v1) % m1, (v2 * v.v2) % m2};
  }
  inline HashValue operator-(const HashValue& r) const
  {
    return HashValue{(v1 + m1 - r.v1) % m1, (v2 + m2 - r.v2) % m2};
  }
  inline bool operator<(const HashValue& r) const
  {
    return v1 == r.v1 ? v2 < r.v2 : v1 < r.v1;
  };
  inline bool operator==(const HashValue& r) const
  {
...
{
  static
      typename conditional<dir, plus<unsigned int>, minus<unsigned int>>::type
          nxt;
...
template <bool dir>
HashValue subHash(const HashValue has[], const unsigned int l,
...
unsigned int count(const unsigned int n, const unsigned int k)
{
  ...
  for (unsigned int i = 1; i + k <= n + 1; i += k)
    *(cur++) = min(subHash<true>(has, i, i + k - 1),
                   subHash<false>(rhas, i, i + k - 1));
  sort(tmp, cur);
