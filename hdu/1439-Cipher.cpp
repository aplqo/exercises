...
struct replace {
  replace() = default;
  replace(const unsigned int n) { iota(to, to + n, 0); }
  ...
  inline unsigned int& operator[](const unsigned int i) { return to[i]; }
  inline unsigned int operator[](const unsigned int i) const { return to[i]; }
};
