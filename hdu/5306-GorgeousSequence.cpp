...
char inbuf[maxn * 10 + 100 * 40 + maxm * 40 + 1000],
    *inpos = inbuf, outbuf[maxm * 11 + 1000], *outpos = outbuf;
...
class SegmentTree {
 public:
  void build(const unsigned int l, const unsigned int r,
             const unsigned int val[], SegmentTree *(&cur));
  void update(const unsigned int l, const unsigned int r,
