...
unsigned int dfsQuery(const unsigned int x, const unsigned int fa)
{
  for (edge* i = head[x]; i; i = i->pre)
    if (i->to != fa) qCnt[x] += dfsQuery(i->to, x);
...
void dfs(const unsigned int x, const unsigned int fa, unsigned int v[],
         unsigned int& vCnt)
{
  insert(a[x], v, vCnt);
  if (cCnt[x]) ans += cCnt[x] * vCnt;
  if (qCnt[x] > 1) {
    for (edge* i = head[x]; i; i = i->pre) {
      if ((!qCnt[i->to] && !cCnt[i->to]) || i->to == fa) continue;
      copy(v, v + vCnt, stk[x]);
      cnt[x] = vCnt;
...
void solveQuery(const unsigned int qn, const unsigned int n)
{
  ...
  for (unsigned int i = 1; i <= n; ++i) {
    ...
    tCnt = 0;
    if (dfsQuery(i, 0)) dfs(i, 0, tmp, tCnt);
  }
}
