/*
 * Luogu team T127959: Connection number
 */
#ifdef APTEST
...
struct edge {
  ...
  {
    return this->from.ps != r.from.ps ? this->from.ps < r.from.ps
                                      : this->to.ps < r.to.ps;
  }
  inline bool operator==(const edge& r) const
  {
...
unsigned int dfs(scc* const x, const unsigned int sta)
{
  ...
  x->vis = sta;
  for (auto i = x->head; i; i = i->pre)
    if (i->to.ps->vis != sta) ret += dfs(i->to.ps, sta);
  return ret;
}
inline void topological()
{
  for (unsigned int i = 0; i < sen - sc; ++i) {
    ...
    for (unsigned int j = 0; j < sen - sc; ++j)
      if (sc[i].c.test(j)) sc[i].con += sc[j].cnt;
  }
}
