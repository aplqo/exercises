...
struct node {
  node() = default;
  inline node(unsigned int num, node* pre) : num(num), pre(pre), vis(stamp) {}
  unsigned int num;
  unsigned int vis = 0;
  node* pre = nullptr;
...
inline void bfs(const unsigned int n, const unsigned int m)
{
  ...
  for (unsigned int* i = digit; i < digit + m; ++i) {
    ...
    if (!*i) continue;
    if (f[to].vis != node::stamp || f[to].num > *i) f[to] = node(*i, nullptr);
  }
  for (unsigned int i = 0; i < min<unsigned int>(n, 10); ++i)
    if (f[i].vis == node::stamp) q.push(i);
  while (!q.empty()) {
    ...
    for (unsigned int* i = digit; i < digit + m; ++i) {
      ...
      if (f[to].vis == node::stamp) continue;
      f[to] = node(*i, pn);
      q.push(to);
    }
  }
