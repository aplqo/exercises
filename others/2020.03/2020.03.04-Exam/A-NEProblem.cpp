...
inline void euler(const unsigned int n)
{
  for (unsigned int i = 2; i <= n; ++i) {
    ...
    for (factor* j = res; j < pcur && j->val <= mindiv[i] && j->val * i <= n;
         ++j)
...
inline void fact(num_t v)
{
  ...
  if (v > 1) {
    factor* p =
        find_if(pcur, cur, [&v](const factor& i) { return i.val == v; });
    if (cur == p) {
      p->val = v;
      p->tmp[0] = p->tmp[1] = 0;
      ++cur;
...
inline bool calc()
{
  ...
  for (factor* i = res; i < cur; ++i) {
    if (i->tmp[OPER] == i->tmp[RES])
      ...
      if (i->fixed && i->ans[LEFT] != i->tmp[RES]) return false;
      i->fixed = true;
...
inline void solve()
{
  ...
  fact<OPER>(a0);
  fact<RES>(a1);
  if (!calc<greater<num_t>, LEFT>()) goto fail;
  fact<OPER>(b0);
  fact<RES>(b1);
  if (!calc<less<num_t>, RIGHT>()) goto fail;
  for (factor* i = res; i < cur; ++i) {
    if (i->ans[LEFT] > i->ans[RIGHT]) goto fail;
