...
unsigned int greedy(const unsigned int n, const unsigned int k,
                    unsigned long long m)
{
  priority_queue<unsigned int, vector<unsigned int>, greater<unsigned int>>
      heap;
  sort(obj, obj + n,
       [](const Object& a, const Object& b) { return a.q < b.q; });
  unsigned int ret = 0;
  ...
  if (ret < k || !m) return ret;
  sort(obj + k, obj + n,
       [](const Object& a, const Object& b) { return a.p < b.p; });
  for (const Object* i = obj + k; i < obj + n && m; ++i) {
    const unsigned int v1 = i->p, v2 = heap.top() + i->q;
    if (v1 > m && v2 > m) continue;
    m -= min(v1, v2);
    if (v2 < v1) heap.pop();
    ++ret;
  }
