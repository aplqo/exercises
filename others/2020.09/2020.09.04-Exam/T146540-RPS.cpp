...
bool buildDfs(const Type root, const unsigned int dep, char* str)
{
  ...
  return buildDfs(root, dep - 1, str) &&
         buildDfs(win[root], dep - 1, str + (1 << (dep - 1)));
}
...
bool build(const Type root, const unsigned int r, const unsigned int p,
           const unsigned int s)
{
  ...
  if (!buildDfs(root, Log2(r + p + s), buf)) return false;
  SortStr(0, r + p + s, buf);
