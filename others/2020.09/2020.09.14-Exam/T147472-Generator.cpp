...
void createStore(ofstream& in, const unsigned int n, const unsigned int mc,
                 const unsigned mv, const unsigned int mt)
{
  uniform_int_distribution<unsigned int> disc(1, mc), disv(1, mv), dist(1, mt);
  for (unsigned int i = 0; i < n; ++i)
    in << disc(default_rnd) << " " << disv(default_rnd) << " "
       << dist(default_rnd) << "\n";
}
void createQuery(ofstream& in, const unsigned int m, const unsigned int mc,
...
int main(int, char* argv[])
{
  ...
  ofstream in(argv[6]);
  in << n << " " << m << "\n";
  const unsigned int mc = MayBe(atoi(argv[3]), maxc),
                     mt = MayBe(atoi(argv[5]), maxt);
