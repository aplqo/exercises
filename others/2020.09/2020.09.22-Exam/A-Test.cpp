...
enum Ret { Accept = 0, Count = 1, Output = 2, Range = 3, Sum = 4 };
int a[maxn + 10];
...
void testSum(ifstream& out, const unsigned int n, const unsigned int cn)
{
  ...
  for (unsigned int i = 0; i < cn; ++i) {
    const unsigned int a = ReadOutput<unsigned int>(out),
                       b = ReadOutput<unsigned int>(out);
    if (a > b) {
  ...
  for (unsigned int i = 1; i <= n; ++i)
    if (seq[i] != a[i]) {
      cout << "Sum not equal. a[" << i << "]=" << a[i] << " out[" << i
           << "]=" << seq[i] << "\n";
      exit(Sum);
    }
}
