...
{
    return Distance { a, b, hypot(static_cast<real>(a->x - b->x), a->y - b->y) / abs(a->t - b->t) };
}
...
Distance binarySearch(const unsigned int n, const unsigned int k, const unsigned int dcnt)
{
    int l = 0, r = dcnt - 1;
    while (l <= r)
    {
