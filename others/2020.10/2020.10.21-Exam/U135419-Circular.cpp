...
static void initNxt(const unsigned int n, const unsigned int vcnt, const unsigned int lg)
{
    static unsigned int suf[maxn * 4 + 10];
    const auto pred = [](const unsigned int a, const unsigned int b) { return seg[a].r < seg[b].r; };
    fill(reinterpret_cast<unsigned int*>(nxt), reinterpret_cast<unsigned int*>(nxt) + sizeof(nxt) / sizeof(unsigned int), n);
...
unsigned int binarySearch(unsigned int pos, const unsigned int ex, const unsigned int lg)
{
    unsigned int ret = 0;
