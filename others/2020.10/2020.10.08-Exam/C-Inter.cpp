...
bool isIntersect(const unsigned int a1, const unsigned int a2, const unsigned int b1, const unsigned int b2)
{
    const unsigned int lc1 = lca(a1, a2), lc2 = lca(b1, b2);
    return (isInSubTree(a1, lc2) ^ isInSubTree(a2, lc2))
        || (isInSubTree(b1, lc1) ^ isInSubTree(b2, lc1))
        || lc1 == lc2;
}
int main()
{
    ...
    for (unsigned int i = 0; i < q; ++i)
    {
        ...
        cin >> a >> b >> c >> d;
        cout << (isIntersect(a, b, c, d) ? "YES" : "NO") << "\n";
    }
    return 0;
}