...
struct Point
{
    int x, y;
    template <class T>
    T& operator[](T dat[3][3]) const { return dat[x][y]; }
};
struct Step
{
    int dx, dy;
...
void linePointer(unsigned int* ptr[], const unsigned int pla, Point p, const Step ini)
{
...
void rotate(unsigned int* const* const ptr, const unsigned int len, const unsigned int offset)
{
...
inline void rotateLeft(unsigned int* const* const pl, const unsigned int len, const unsigned int off)
{
    rotate(pl, len, len - off);
}
inline void roteteRight(unsigned int* const* const pl, const unsigned int len, const unsigned int off)
{
    rotate(pl, len, off);
}
