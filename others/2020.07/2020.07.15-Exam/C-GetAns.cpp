...
struct visGuard {
  visGuard(unsigned int r, unsigned int c) : r(r), c(c) { vis[r][c] = true; }
  ~visGuard() { vis[r][c] = false; }
  const unsigned int r, c;
};
unsigned long long dfs1(const unsigned int r, const unsigned int c,
                        const bool typ)
{
  if (c > m || r > n || (vis[r][c] && (r != 1 || c != 1)) || fail[r][c])
    return 0;
  if (c == m && r == n) return typ ? dfs1(1, 1, false) : 1;
  visGuard g(r, c);
