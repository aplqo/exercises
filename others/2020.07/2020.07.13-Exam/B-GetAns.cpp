...
#define decl(x) inline friend Number operator x (const Number l, const Number r); \
...
#define impl(x) inline Number operator x(const Number l,const Number r){return (l.val+mod) x r.val; }
impl(+);
impl(-);
impl(*);
#undef impl
...
Number solve(const unsigned int n,const unsigned long long t,const Number inv)
{
