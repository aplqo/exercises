...
constexpr unsigned long long mod[] = {(unsigned long long)1e9 + 7,
                                      (unsigned long long)1e9 + 9},
                             base = 7;
...
template <unsigned long long mod>
void getHash(const char* str, const unsigned int len,
             unsigned long long res[26][maxl + 1])
{
...
inline unsigned long long subHash(const unsigned int ch, const unsigned int l,
                                  const unsigned int r)  // [l,r]
{
  return (has[typ][ch][r] + mod[typ] -
          (has[typ][ch][l - 1] * po[typ][r + 1 - l]) % mod[typ]) %
         mod[typ];
}
...
bool check(const unsigned int pl, const unsigned int pr, const unsigned int len)
{
  ...
  for (unsigned int i = 0; i < 26; ++i) {
    hl[i] =
        Hash{subHash<0>(i, pl, pl + len - 1), subHash<1>(i, pl, pl + len - 1)};
    hr[i] =
        Hash{subHash<0>(i, pr, pr + len - 1), subHash<1>(i, pr, pr + len - 1)};
  }
  sort(hl, hl + 26);
  sort(hr, hr + 26);
