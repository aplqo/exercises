...
namespace bf {
pair<Number, Number> dfs(const unsigned int n, const unsigned long long dep,
                         const unsigned int g, const Number v)
{
  if (!dep) return make_pair(v, g);
...
int main()
{
  ...
  for (unsigned int i = 0; i < t; ++i) {
    ...
    cin >> n >> k;
    cout << [](unsigned int n,
               unsigned long long k) { return bf::solve(n, k); }(n, k)
         << "\n";
  }
  return 0;
}