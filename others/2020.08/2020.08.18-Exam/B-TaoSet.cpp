...
namespace dsu
{
    ...
    inline long long query(const unsigned int x)
    {
        return val[x] + acc[find(x)];
    }
    ...
    inline void add(const unsigned int x, const long long v)
    {
        acc[find(x)] += v;
    }
}
...
void STree::reset(const unsigned int l, const unsigned int r, const unsigned int s)
{
    if (l == lv && r == rv)
    {
        push(s);
...
inline void addQuery(const unsigned int sta, const unsigned int l, const unsigned int r)
{
...
{
    sort(qry, cur, [](const QueryPoint& a, const QueryPoint& b) { return a.sta < b.sta; });
    QueryPoint* p = qry;
