...
bool check(const unsigned int x, const unsigned int m)
{
  ...
  for (const Con* i = c; i < c + m; ++i)
    if (dis[i->a] + dis[i->b] - 2 > i->d) return false;
  return true;
}
unsigned int findVert(const unsigned int n, const Con* c)
{
  ...
  for (unsigned int i = 1; i <= n; ++i)
    if (disa[i] + disb[i] - 2 <= c->d && (!ret || dep[i] < dep[ret])) ret = i;
  return ret;
}
...
int main()
{
  ...
  const unsigned int p = findVert(
      n, find_if(c, c + m, [w](const Con& i) { return i.minDep() == w; }));
  if (!p || !check(p, m)) {
    cout << "NO" << '\n';
    return 0;
  }
