...
struct fun
{
    inline unsigned int operator()(const trie& t, const unsigned int pos, const unsigned int m)
    {
        unsigned int ret = 0;
...
#define test(s, m) (static_cast<bool>((s) & (m)))
    ...
    constexpr uint_fast32_t msk(const unsigned int v)
    {
        return 0xff << (v * 8);
    }
