...
unsigned long long c(const unsigned int m, const unsigned int n)
{
    if (!n || n >= m)
        return 1;
    return (factor(m - n, n) * (QuickPow(factor(0, n), mod - 2))) % mod;
}
...
int main()
{
    ...
    {
        static auto fun = [](const unsigned int i) { return QuickPow(2, i * (i - 1) / 2); };
        f[i] = fun(i);
        for (unsigned int j = 1; j < i; ++j)
        {
            unsigned long long cv = (c(i - 1, j - 1) * f[j]) % mod;
            cv = (cv * fun(i - j)) % mod;
            f[i] = (f[i] + mod - cv) % mod;
        }
    }
