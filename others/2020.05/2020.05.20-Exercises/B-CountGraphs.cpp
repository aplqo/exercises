/*Luogu team T134033: Count graphs*/
#ifdef APTEST
...
unsigned long long c(const unsigned int m, const unsigned int n)
{
    ...
    if (mem[m][n])
        return mem[m][n];
    return mem[m][n] = (factor(m - n, m) * (QuickPow(factor(0, n), mod - 2))) % mod;
}
...
int main()
{
    ...
    {
        static const auto fun = [](const unsigned int i) { return QuickPow(2, i * (i - 1) / 2); };
        f[i] = fun(i);
