...
template <class T>
void dfs(const unsigned int* const p, T fun, const unsigned int dep = 0, const unsigned long long cur = 1)
{
    if (p >= ten)
    {
        if (dep)
...
unsigned int delt(const unsigned int v)
{
    if (v == 1)
        return cnt - 1;
    long long ret = 0;
    dfs(tmp, [&ret](unsigned int d, unsigned long long c) { ret = d & 0x01 ? ret + fac[c] : ret - fac[c]; });
    return cnt - ret;
}
