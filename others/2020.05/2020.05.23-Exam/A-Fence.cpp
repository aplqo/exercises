...
enum typ { Left = 0, Right = 1, Unknown = 0xff };
...
{
  const static auto upd = [](typ& v, typ& t) {
    if (t != Unknown)
      v = t;
    else if (v != Unknown)
      t = v;
  };
  const static auto point = [](const ed& p, dir io) {
    return toPoint(p.first, p.second, io);
  };
  if (l > r) swap(l, r);
...
unsigned int solve(const unsigned int n)
{
  ...
  for (unsigned int i = 0; i < mp; ++i)
    for (unsigned int j = 0; j < mp; ++j)
      if (j != i) ret = min(ret, f[i][j] + f[j][i]);
  return ret;
}
int main()
{
