...
ostream& operator<<(ostream& os, dpoint s)
{
    ...
    os << (get<1>(s) == Left ? "L" : "R");
    os << (get<2>(s) == In ? "I" : "O");
    os << " (" << (get<0>(s) * 4 + get<1>(s) + get<2>(s)) << ")";
    return os;
}
void insEdge(const point u, const point v, const dir ud = Out, const dir vd = In)
{
    dpoint u1 = make_tuple(u.first, u.second, ud);
    dpoint u2 = make_tuple(v.first, v.second, vd);
    edge&& e = make_pair(u1, u2);
    if (vec.find(e) == vec.end())
        vec.insert(e);
}
void addEdge(point l, point r)
{
    static const auto upd = [](typ& t, typ v) {if(v!=Unknown)t=v; };
    if (l.first > r.first)
        swap(l, r);
...
{
    const static auto read = []() -> unsigned int { unsigned int i; cin>>i; return i-1; };
    unsigned int n1, n2;
    cin >> n1 >> n2;
    insEdge(make_pair(s, Left), make_pair(s, Left), In, Out);
    insEdge(make_pair(s, Right), make_pair(s, Right), In, Out);
