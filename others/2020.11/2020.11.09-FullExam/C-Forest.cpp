...
struct State {
  ...
  template <class T>
  inline T& operator[](T dat[maxn + 10][maxm + 10]) const
  {
    return dat[x][y];
  }
};
enum class Visit { Not = 0, Visited, Mark };
...
unsigned int bfs(const int n, const int m, const int sx, const int sy)
{
  ...
  enum Update { Not = 0, Up = 0b01, Down = 0b10 };
  static unsigned int upd[maxn + 10][maxm + 10];
  queue<State> q;
  q.push(State{sx, sy});
  if (vis[sx][sy] != Visit::Mark) q.front()[vis] = Visit::Visited;
  unsigned int ret = UINT_MAX;
  while (!q.empty()) {
    const State cur = q.front();
    q.pop();
    if (cur[vis] == Visit::Mark) {
      if (cur[upd] == (Update::Up | Update::Down)) ret = min(ret, cur[dist]);
      continue;
    }
...
pair<unsigned int, unsigned int> readMatrix(const unsigned int n,
...
unsigned int findMin(const unsigned int n, const unsigned int m,
                     const unsigned int sx, const unsigned int sy)
{
  for (unsigned int i = 1; i <= n; ++i) {
    const unsigned int pos =
        find(isForest[i] + 1, isForest[i] + 1 + m, true) - isForest[i];
    if (pos == 1 + m) continue;
