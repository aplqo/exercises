...
pair<bool, unsigned long long> greedy(const unsigned int n,
                                      const unsigned int x, const int y,
                                      const unsigned long long e, long long h)
{
  priority_queue<long long> q;
  unsigned long long acc = 0, mx = 0;
  for (unsigned int i = 1; i <= n; ++i) {
    acc += x;
    if (acc >= e) return make_pair(true, i);
    ...
    if (h <= 0) return make_pair(false, mx);
  }
