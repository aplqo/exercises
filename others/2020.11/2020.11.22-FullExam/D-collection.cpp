...
inline long long index(const unsigned int x) { return tag[find(x)] + c[x]; }
void merge(unsigned int x, unsigned int y)
{
  x = find(x);
  y = find(y);
...
void add(unsigned int x, unsigned int delt) { tag[find(x)] += delt; }
...
int main()
{
  ...
  long long* cqr = ans;
  {
    ...
    sort(pqr, cpq,
         [](const PartQuery& a, const PartQuery& b) { return a.id < b.id; });
