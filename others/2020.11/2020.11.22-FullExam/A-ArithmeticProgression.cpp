...
pair<long long, long long> jumpRange(const long long l, const long long r, const long long x, long long dx)
{
    const bool pos = dx >= 0;
    if (!pos)
        dx = -dx;
...
unsigned long long count(const int a1, const int d1, const int a2, const int d2, const long long c, const long long d)
{
    ...
    const auto [xl, xr] = jumpRange(solveEqu(a1, d1, c), solveEqu(a1, d1, d), x, d2 / g);
    const auto [yl, yr] = jumpRange(solveEqu(a2, d2, c), solveEqu(a2, d2, d), y, d1 / g);
