...
bool dfs(const unsigned int pos, const unsigned int bus, const unsigned int rem,
         const unsigned int k)
{
  if (!bus || sm < sw) return false;
  if (!k || rem >= sw) return true;
  if (!pos || rem < mnw) {
    sm -= rem;
    if (dfs(findK(k), bus - 1, mon[bus - 1], k)) return true;
    sm += rem;
    return false;
  }
  for (int i = min({rem / pos, cnt[pos], k}); i >= 0; --i) {
    ...
    sw -= pos * i;
    sm -= pos * i;
    if (dfs(pos - 1, bus, rem - i * pos, k - i)) return true;
    sm += pos * i;
    sw += pos * i;
    cnt[pos] += i;
  }
...
{
  if (sumw[k] > msum || w[k] > mon[n]) return false;
