...
void xorAdd(const unsigned int x1, const unsigned int y1, const unsigned int x2,
            const unsigned int y2, const unsigned long long v)
{
  prefixXor(x1, y1, v);
  prefixXor(x1, y2 + 1, v);
  prefixXor(x2 + 1, y1, v);
...
inline unsigned long long getRandom() { return dis(rnd); }
}  // namespace Rand

inline void addSquare(const unsigned int x1, const unsigned int y1,
...
inline void removeSquare(const unsigned int x1, const unsigned int y1,
                         const unsigned int x2, const unsigned int y2)
{
  ...
  for (unsigned int i = 0; i < q; ++i) {
    ...
    switch (p) {
      ...
      case 2:
        ...
        break;
      case 3:
        cout << (FTree::prefixSum(x1, y1) == FTree::prefixSum(x2, y2) ? "Yes"
                                                                      : "No")
             << "\n";
        break;
    }
