...
unsigned long long countMedian(const unsigned int n, const unsigned int k, const unsigned int l2, const unsigned int r2) // [pl, pr), pl only in prefix sum
{
    ...
    {
        if (i - lp > r2)
            FTree::change(sum[lp++] + n + 1, n * 2 + 5, -1);
