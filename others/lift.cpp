...
{
    partial_sum(ans[i] + beg, ans[i] + end + 1, sum[i] + beg, [](const res_t a, const res_t b) -> res_t { return (a + b) % mod; });
}
...
inline void solveL(const res_t b, const res_t k)
{
    ...
    for (res_t i = 1; i <= k; ++i)
    {
        ...
        {
            ans[pos][j] = (sum[lst][b - 1] - (2 * j - b >= 1 ? sum[lst][2 * j - b] : 0) + mod) % mod;
...
inline void solveG(const res_t n, const res_t b, const res_t k)
{
    ...
    for (res_t i = 1; i <= k; ++i)
    {
        const res_t pos = i & 0x01, lst = pos ^ 0x01;
        for (res_t j = beg; j < end; ++j)
            ans[pos][j] = ((2 * j - b - 1 > n ? sum[lst][n] : sum[lst][2 * j - b - 1]) - ans[lst][j] + mod) % mod;
        getSum(pos, beg, end);
    }
}
int main()
