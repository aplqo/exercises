...
struct Node
{
    unsigned long long greaterCount(const unsigned long long val, const unsigned long long lim) const;
    unsigned long long dfsSelectOne(const Node* rt, const unsigned long long acc, const unsigned long long x) const;
    Number dfsSubseq(const unsigned long long acc, const unsigned long long x) const;
...
unsigned long long Node::greaterCount(const unsigned long long val, const unsigned long long lim) const
{
    if (isEnd)
        return (val & 0x01) >= (lim & 0x01) ? size : 0;
    unsigned int ret = 0;
    const Node* ptr = this;
...
unsigned long long Node::dfsSelectOne(const Node* rt, const unsigned long long acc, const unsigned long long x) const
{
    if (isEnd)
        return rt->greaterCount(acc, x) * size;
    unsigned long long ret = 0;
    for (unsigned long long i = 0; i < 2; ++i)
        if (chl[i])
            ret += chl[i]->dfsSelectOne(rt, acc | (i << dep), x);
    return ret;
}
Number Node::dfsSubseq(const unsigned long long acc, const unsigned long long x) const
{
