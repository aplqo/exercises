...
enum { Accept = 0, State = 1, OutOfRange = 2, WrongWeight = 3, Output = 4 };
enum class state { Win, Draw, Lose };
struct Edge {
  unsigned int to, val;
  const Edge* pre;
} ed[maxn + maxk + 10];
...
inline Edge* addEdge(const unsigned int from, const unsigned int to,
                     const unsigned int val) noexcept
{
  static Edge* cur = ed;
...
void dijkstra(const unsigned int n, const unsigned int s) noexcept
{
  ...
  while (!q.empty()) {
    ...
    q.pop();
    if (vis[cur.second]) vis[cur.second] = true;
...
void readInput(istream& is, const unsigned int n, const unsigned int m,
...
void readVarEdge(ifstream& out, const unsigned int k)
{
  for (VarEdge* i = vr; i < vr + k; ++i) {
    ...
    if (i->l <= val && val <= i->r)
      ...
      print(cout, "Edge ", i - vr, " weight out of range. Read=", val,
            " Range=[", i->l, ",", i->r, "]");
      exit(OutOfRange);
    }
  }
...
void checkDistance(const unsigned int n, const state equ, const unsigned int s1,
                   const unsigned int s2, const unsigned int f)
{
  if (equ == state::Lose) return;
  dijkstra(n, f);
  if (equ == state::Win && !(dis[s1] < dis[s2])) {
    print(cout, "Wrong edge weight. Expect win. Actually dist[", s1,
          "]=", dis[s1], " dist[", s2, "]=", dis[s2]);
    exit(WrongWeight);
  }
  else if (equ == state::Draw && !(dis[s1] == dis[s2])) {
    print(cout, "Wron edge weight. Expect draw. Actually dist[", s1,
          "]=", dis[s1], " dist[", s2, "]=", dis[s2]);
    exit(WrongWeight);
  }
}
