...
inline Ploy operator-(const int l, const Ploy r) { return Ploy{-r.k, l - r.b}; }
...
void dfsEquation(const unsigned int x, const unsigned int root)
{
  ...
  for (const Edge* i = head[x]; i; i = i->pre) {
    ...
    if (vis[to]) {
      ...
      if (ek && !eb ||
          equal(p[to].k, 0) && !equal(p[to].b + p[x].eval(p[to].b), i->val)) {
        cout << "NO\n";
        exit(0);
      }
      else if (ek && eb)
...
void solveBlock(const unsigned int root)
{
  ...
  for_each(vert.cbegin(), vert.cend(),
           [x = p[root].k ? minAbsolute() : p[root].b](unsigned int i) {
             ans[i] = p[i].eval(x);
           });
  vert.clear();
}
