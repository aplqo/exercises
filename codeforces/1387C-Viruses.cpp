...
#include <queue>
using namespace std;
const unsigned int maxl = 50, maxk = 100, maxn = maxk, maxg = maxn + 2, maxm = 50;
...
public:
    ...
    inline void add(const unsigned int from, const unsigned int to, const unsigned int another)
    {
...
struct StateNode
{
    ...
    template <class T>
    inline T& operator[](StateMatrix<T> mat) const { return mat[id][start][end]; }
};
struct StateLess
{
...
void dijkstra(const unsigned int acId)
{
    ...
    Heap q;
    fill(reinterpret_cast<Length*>(dis), reinterpret_cast<Length*>(dis) + sizeof(dis) / sizeof(Length), inf);
    dpInit(q);
    ...
    while (!q.empty())
    {
        ...
        for (auto i = asFirst.head[cur.id]; i; i = i->pre)
            for (unsigned int j = 0; j < acId; ++j)
                update(StateNode { i->to, cur.start, j, cur.dis + dis[i->another][cur.end][j] });
        for (auto i = asSecond.head[cur.id]; i; i = i->pre)
            for (unsigned int j = 0; j < acId; ++j)
                update(StateNode { i->to, j, cur.end, dis[i->another][j][cur.start] + cur.dis });
    }
}
