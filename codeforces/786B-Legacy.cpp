...
const unsigned int maxv = maxq * maxl * 2 + maxn * 4 * 2, maxe = maxq * maxl * 2 + maxq;
...
void addEdge(const unsigned int from, const unsigned int to, const unsigned long long val = 0)
{
    static edge* cur = ed;
    cur->to = to;
    cur->val = val;
...
template <unsigned int t>
void add(const unsigned int rt, const Range cr, const Range qr, const unsigned int vp, const unsigned long long w)
{
    if (cr == qr)
    {
        if constexpr (t == In)
...
void addEdge(const Range from, const unsigned int to, const unsigned long long w)
{
    const unsigned int p = ++vCnt;
    add<In>(root[In], Range(1, n + 1), from, p, w);
    addEdge(p, pos[Out][to]);
}
void addEdge(const unsigned int from, const Range to, const unsigned long long w)
{
    const unsigned int p = ++vCnt;
