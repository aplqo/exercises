...
static void eular(const unsigned int n)
{
    for (unsigned int i = 2; i <= n; ++i)
    {
        ...
        for (const unsigned int* j = primes; j < pcur && 1ull * i * (*j) <= n && *j <= mindiv[i]; ++j)
            mindiv[*j * i] = *j;
    }
}
...
pair<unsigned long long, unsigned long long> factor(unsigned int x, const unsigned int k)
{
    unsigned long long hashX = 0, hashC = 0; // hash of x, hash of i ( i * x == t ^k)
    while (x > 1)
    {
