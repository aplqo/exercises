...
{
public:
    void build(const unsigned int l, const unsigned int r, STree*(&cur), const unsigned int v[]);
...
void STree::build(const unsigned int l, const unsigned int r, STree*(&cur), const unsigned int v[])
{
    lv = l;
    rv = r;
    mv = (l + r) >> 1;
...
unsigned int addChild(const unsigned int x, const unsigned int f)
{
    ...
    for (auto i = tree.head[x]; i; i = i->pre)
        if (i->to != f)
            vals[x].insert(cost[i->to]);
    return *vals[x].begin();
}
namespace Chain
{
    unsigned int fa[maxv + 1], top[maxv + 1], son[maxv + 1], dep[maxv + 1], dfn[maxv + 1];
