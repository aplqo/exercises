...
static void initSum(const unsigned int n, const unsigned long long a[], unsigned long long res[])
{
...
unsigned long long findMax(const unsigned int n)
{
    ...
    {
        const unsigned int x = (i & 0x03) == 1 || (i & 0x03) == 2, y = i / 2 + 1, typ = i & 0x01;
        acc += val[x][y] * i;
        ret = max(ret, acc + i * sum[x][y + 1] + gain[x][y + 1][Pre] + (n + i - y) * sum[!x][y + typ] + gain[!x][y + typ][Suf]);
    }
    return ret;
}
int main()
