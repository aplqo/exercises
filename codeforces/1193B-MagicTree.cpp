...
void correctFruit(Map& mp, const Map::iterator vp, const unsigned long long rootJuice)
{
...
void dfs(const unsigned int x)
{
    ...
    if (fru[x].exist)
    {
        ...
        else if (it->first == f.day)
        {
            ...
            correctFruit(heavest[x], heavest[x].insert({ f.day, f.juice }).first, f.juice);
    }
}
...
int main()
{
    ...
    dfs(0);
    cout << accumulate(heavest[0].begin(), heavest[0].end(), 0ull, [](unsigned long long ac, Map::value_type v) {
        return ac + v.second;
    }) << "\n";
    return 0;
}