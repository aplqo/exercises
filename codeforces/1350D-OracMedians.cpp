...
enum class Ord { Less, Equal, Greater } trans[maxn + 10];

bool check(const unsigned int n)
{
  if (n == 1) return trans[1] == Ord::Equal;
  unsigned int lst = 0;
  const bool existEq =
      any_of(trans + 1, trans + 1 + n, [](Ord a) { return a == Ord::Equal; });
  bool existGe = false;
  for (const auto* i = trans + 1; i < trans + 1 + n; ++i)
    if (*i != Ord::Less) {
      if (existEq && existGe && i - trans - lst <= 2) return true;
      existGe = true;
      lst = i - trans;
    }
  return false;
