...
void checkSolution(std::ifstream& out, const std::size_t n)
{
  ...
  for (unsigned int i = 1; i < n; ++i) {
    ...
    if (cur > n)
      fail(ExitStatus::Range, "Wrong solution. ans[", i, "] = ", cur,
           " is greater than |S| = ", n);
    if (vis[cur])
      fail(ExitStatus::NotPermutation, "Wrong solution. ", cur,
           " visited twice. ans[", vis[cur], "] = ans[", i, "] = ", cur);
    if (dat[cur] == dat[lst])
      fail(ExitStatus::Same, "Wrong solution. Adjacent step is same. type[",
           lst, "] = type[", cur, "] = ", dat[cur]);
    lst = cur;
    vis[cur] = i;
  }
}
