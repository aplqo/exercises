...
static void buildEquation(const unsigned int n, const unsigned int m)
{
  ...
  for (const Edge* i = ed; i < ed + m; ++i) {
    if (i->from != n - 1 && i->from != 0) --equ[i->from][i->to];
    if (i->to != n - 1 && i->to != 0) --equ[i->to][i->from];
  }
  equ[0][0] = equ[n - 1][n - 1] = 1;
  equ[0][n] = 0;
  equ[n - 1][n] = 1;
}
inline bool isZero(const Real v) { return fabs(v) <= eps; }
void gaussian(const unsigned int n)
{
  for (unsigned int i = 0; i < n; ++i) {
    ...
    if (isZero(equ[i][i])) continue;
    for (unsigned int j = i + 1; j < n; ++j) {
      if (isZero(equ[j][i])) continue;
