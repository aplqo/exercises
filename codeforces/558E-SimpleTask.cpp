...
std::pair<unsigned int*, unsigned int*> SortedInterval::split(unsigned int idx) const
{
    ...
    return order == Order::Increase ? std::make_pair(ret, count) : std::make_pair(count, ret);
}
void SortedInterval::print() const
{
    const int step = order == Order::Increase ? 1 : -1;
    for (int i = order == Order::Increase ? 0 : maxc - 1; i < maxc && i >= 0; i += step)
