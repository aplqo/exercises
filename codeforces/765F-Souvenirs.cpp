...
template <class T, class Pred>
template <T ini>
void SegmentTree<T, Pred>::build(const unsigned int l, const unsigned int r,
                                 SegmentTree *(&cur))
{
  lv = l;
  rv = r;
...
template <class T, class Pred>
T SegmentTree<T, Pred>::extremum(const unsigned int l,
...
template <class T, class Pred>
void SegmentTree<T, Pred>::update(const unsigned int l, const unsigned int r,
                                  const T val)
{
  if (l == lv && r == rv) {
    laz = min(laz, val, prd);
...
void answerPart(const unsigned int n, const unsigned int dcnt)
{
  ...
  idxTree *const idxRoot =
      buildTree<unsigned int, idxTree, 0>(0, dcnt, idxNodes);
  ansTree *const ansRoot =
      buildTree<unsigned long long, ansTree, ullInf>(0, n + 1, ansNodes);
  for (unsigned int i = 1; i <= n; ++i) {
    {
      ...
      while (l < w[i]) {
        ...
        l = lower_bound(orig, orig + dcnt, (orig[w[i]] + orig[w[p]] + 1) / 2) -
            orig;
      }
