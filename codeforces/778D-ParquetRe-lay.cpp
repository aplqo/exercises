...
private:
    ...
    bool change(const unsigned int x, const unsigned int y)
    {
        ...
        if constexpr (typ == L)
        {
            if (val[x][y + 1] != U && !change<U>(x, y + 1))
                return false;
        }
        else
        {
            if (val[x + 1][y] != L && !change<L>(x + 1, y))
                return false;
        }
        rotate(x, y);
        return true;
...
int main()
{
    ...
    unsigned int n, m;
    cin >> n >> m;
    const auto [suc, seq] = m & 0x01 ? constructSol<true>(n, m) : constructSol<false>(n, m);
    if (!suc)
        cout << "-1\n";
    else
    {
