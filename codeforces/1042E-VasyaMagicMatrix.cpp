...
#undef decl
    ...
    friend inline ostream& operator<<(ostream& os, const Number v) { return os << v.val; }
...
struct Point
{
    ...
    inline bool operator<(const Point r) const { return dat[x][y] < dat[r.x][r.y]; }
    template <class T>
...
void dp(const unsigned int cnt)
{
    ...
    for (; i < cnt; ++i)
    {
        ...
        cp[exp] = (square[lstp] - Number(2) * cp.x * sx[lstp] - Number(2) * cp.y * sy[lstp] + se[lstp]) / (lstp + 1) + cp.distOrig();
        sx[i] = sx[i - 1] + cp.x;
        sy[i] = sy[i - 1] + cp.y;
