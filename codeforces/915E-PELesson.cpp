...
class SegmentTree {
 public:
  ...
  unsigned int assign(const unsigned int l, const unsigned int r,
                      const bool val);
...
SegmentTree SegmentTree::nodes[maxq * 4 * 2 + 10],
    *SegmentTree::cur = SegmentTree::nodes + 1;
struct Query {
  unsigned int l, r, k;
} qry[maxq + 10];
...
unsigned int SegmentTree::assign(const unsigned int l, const unsigned int r,
                                 const bool val)
{
  if (l == lv && r == rv) {
    dirty = true;
