...
#include <queue>
using namespace std;
const unsigned int maxsn = 5e4, maxm = 5e4, maxc = 26, maxl = 21, maxs = 5e5,
                   maxq = 5e5, maxt = 5e4;
...
SegmentTree SegmentTree::nodes[maxNode * 17 + 10],
...
void SegmentTree::increase(const unsigned int pos)
{
  ...
  static const auto orCreate = [](SegmentTree*& p, const unsigned int l,
...
ValFrom SegmentTree::max(const unsigned int l, const unsigned int r) const
{
  ...
  static const auto maybe = [](SegmentTree* p, const unsigned int l,
                               const unsigned int r) {
...
{
  val = ValFrom{0, lv};
  if (lc) val = std::max(val, lc->val);
  if (rc) val = std::max(val, rc->val);
}
namespace GSAM {
struct SAMNode {
...
void insertSAM(SAMNode* const x, const unsigned int ch, SAMNode* last)
{
  ...
  if (!from)
    ...
    if (fNxt->len == from->len + 1)
      ...
      new (cloned) SAMNode{
          from->len + 1, 0, fNxt->link, {}, SegmentTree::create(0, m + 1)};
...
void buildGSAM()
{
  ...
  for (unsigned int i = 0; i < maxc; ++i)
    if (root->next[i]) q.emplace(root, i);
  while (!q.empty()) {
...
void addQuery(const unsigned int l, const unsigned int r, const unsigned int pl,
