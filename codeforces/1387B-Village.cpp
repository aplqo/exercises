...
unsigned int dfsCentroid(const unsigned int x, const unsigned int f,
                         unsigned int& pos, unsigned int& mxs)
{
...
unsigned int dfs(const unsigned int x, const unsigned int f)
{
  unsigned int ret = 0;
  for (const Edge* i = head[x]; i; i = i->pre)
    if (i->to != f) ret += dfs<false>(i->to, x);
  if (seq[x] == x) {
    ret += 2;
    if constexpr (isRoot)
...
#ifdef Minimum
  cout << MinDist::solve(n) << "\n";
  copy(MinDist::seq + 1, MinDist::seq + 1 + n,
       ostream_iterator<unsigned int>(cout, " "));
  cout.put('\n');
#else
  cout << MaxDist::solve(n) << "\n";
  copy(MaxDist::seq + 1, MaxDist::seq + 1 + n,
       ostream_iterator<unsigned int>(cout, " "));
  cout.put('\n');
#endif
  return 0;
