...
void shift(const ConstArrayPtr a, const unsigned int c, const ArrayPtr dest)
{
    ...
    ntt(ConstArrayPtr { u, a.size }, ConstArrayPtr { t, a.size }, ArrayPtr { dest.ptr, a.size * 2 });
...
void stirling(const unsigned int n, Number dest[])
{
    ...
    for (unsigned int i = 1; i <= lgn; ++i)
    {
        ...
        ntt(ConstArrayPtr { tmp, acc + 1 }, ConstArrayPtr { dest, acc + 1 }, ArrayPtr { dest, csz + 2 });
        acc <<= 1;
        if (n & (1u << (lgn - i)))
        {
            acc |= 1;
