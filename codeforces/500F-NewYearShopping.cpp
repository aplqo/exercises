...
typedef pair<unsigned int, unsigned int>
    Object;  // first cost, second happiness
...
void STree::addObject(const unsigned int l, const unsigned int r,
                      const Object obj)
{
  if (l == lv && r == rv) {
    val.push_back(obj);
...
static void initQuery(const unsigned int q)
{
  ...
  for (unsigned int i = 1; i <= maxt && ptr < q; ++i) {
    pos[i] = ptr;
    ptr = upper_bound(seq + ptr, seq + q, i,
                      [](const unsigned int t, const Query* a) {
                        return t < a->time;
                      }) -
          seq;
  }
