...
static unsigned long long getExp()
{
  const unsigned long long e =
      accumulate(pol.begin(), pol.end(), 0ull,
                 [](unsigned long long v, const Poly& p) { return p.a + v; });
  for (Poly& i : pol)
    i.a = e - i.a;
  return e;
}
pair<unsigned long long, unsigned long long> Log(unsigned long long a,
...
unsigned long long solve(const unsigned long long x)
{
  sort(pol.begin(), pol.end(),
       [](const Poly& a, const Poly& b) { return a.a > b.a; });
  while (pol.size() > 1) {
