...
template <bool lr, bool dir>  // lr: true->l, false->r dir:
                              // true->increase,false->decrease
                              int expand(
                                  int l, const int g,
                                  const long long lim)  // init pos, GL/GR, r/l
{
  static conditional_t<lr, greater_equal<long long>, less_equal<long long>> upd,
      chk;                                               // check p[l] >= p[r]
  static conditional_t<dir, plus<int>, minus<int>> nxt;  // move to next pos
  static conditional_t<dir, less_equal<int>, greater_equal<int>>
      brd;  // not exceed GL/GR
  for (int i = nxt(l, 1); brd(i, g) && chk(p[i], lim); i = nxt(i, 1))
    if (upd(p[i], p[l])) return i;
  return l;
}
template <bool dir>
...
{
  generateP(n, t, v);
  const int gl = max_element(p + 1, p + 1 + k) - p,
            gr = min_element(p + k, p + n + 1) - p;
