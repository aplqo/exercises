...
void createQuery(ofstream& in, const unsigned int q, const unsigned int n, const unsigned int mv)
{
    ...
    {
        const unsigned int op = diso(default_rnd), p1 = disp(default_rnd), p2 = disp(default_rnd);
...
{
    Init();
    const unsigned int n = GetRandom(1, MayBe(atoi(argv[1]), maxn)), q = GetRandom(1, MayBe(atoi(argv[2]), maxq));
