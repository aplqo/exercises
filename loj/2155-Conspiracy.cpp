...
unsigned int count(const unsigned int n)
{
  ...
  iota(seq, seq + n, 1);
  sort(seq, seq + n,
       [](unsigned int a, unsigned int b) { return degree[a] > degree[b]; });
  ...
  for (const unsigned int* i = ptr; i < seq + n; ++i)
    for (const unsigned int* j = i + 1; j < seq + n; ++j)
      if (gra[*i].test(*j)) return 0;
  const unsigned int num = ptr - seq;
  unsigned int ret = (ptr - seq < n);
  if (num != 1)
    ret += upper_bound<const unsigned int*>(
               seq, ptr, num - 1,
               [](unsigned int d, unsigned int i) { return d > degree[i]; }) -
           lower_bound<const unsigned int*>(
               seq, ptr, num - 1,
               [](unsigned int i, unsigned int d) { return degree[i] > d; });
