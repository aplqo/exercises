...
namespace RailwayTrip
{
    ...
    static void initRange(const unsigned int n)
    {
        ...
        for (unsigned int i = 1; i <= maxl; ++i)
            ...
            {
                left[j][i] = min(left[left[j][i - 1]][i - 1], left[right[j][i - 1]][i - 1]);
                right[j][i] = max(right[left[j][i - 1]][i - 1], right[right[j][i - 1]][i - 1]);
            }
    }
    tuple<unsigned int, unsigned int, unsigned int> containRange(const unsigned int pos, const unsigned int cov)
    {
        ...
        for (unsigned int i = maxl; i;)
        {
            ...
            const unsigned int cl = min(left[l][mid], left[r][mid]), cr = max(right[l][mid], right[r][mid]);
            if (cov < cl || cr < cov)
            {
                ret |= 1 << mid;
                l = cl;
