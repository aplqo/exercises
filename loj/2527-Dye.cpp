...
Number solve(const unsigned int n, const unsigned int m, const unsigned int s, const unsigned int lgm)
{
    ...
    const unsigned int minm = std::min(m, n / s);
    {
        ...
        for (unsigned int i = 0, cnt = 0; i <= m && cnt <= n; ++i, cnt += s)
        {
            ...
            t[minm - i] = fac[i] * C(m, i) * fac[n] * ifac[n - cnt] * acc * quickPow(Number { m - i }, n - cnt);
            acc *= ifac[s];
        }
    }
    ntt(h, t, h, lgm);
