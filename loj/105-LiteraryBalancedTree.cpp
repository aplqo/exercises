...
void Splay::rotate()
{
  ...
  if (ff) ff->child[ff->child[1] == f] = this;
...
void Splay::pushdown()
{
  ...
  if (child[0]) child[0]->rev ^= true;
  if (child[1]) child[1]->rev ^= true;
  rev = false;
}
void Splay::pushup()
{
  siz = 1;
  if (child[0]) siz += child[0]->siz;
  if (child[1]) siz += child[1]->siz;
}
void Splay::print(unsigned int*(&buf))
{
  pushdown();
  if (child[0]) child[0]->print(buf);
  *(buf++) = val;
  if (child[1]) child[1]->print(buf);
}
