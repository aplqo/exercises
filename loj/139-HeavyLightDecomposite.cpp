...
class SegmentTree {
 public:
  ...
  void add(const unsigned int l, const unsigned int r,
           const unsigned long long delt);
...
void SegmentTree::build(const unsigned int l, const unsigned int r,
                        SegmentTree*(&cur), const unsigned long long val[])
{
  lv = l;
  rv = r;
  mv = (l + r) >> 1;
...
unsigned long long SegmentTree::sum(const unsigned int l,
...
void SegmentTree::add(const unsigned int l, const unsigned int r,
                      const unsigned long long delt)
{
  if (l == lv && r == rv) {
    laz += delt;
...
{
  if (dfn[x] != 1) fun(1, dfn[x]);
  if (dfn[x] + siz[x] < n + 1) fun(dfn[x] + siz[x], n + 1);
}
template <class T>
...
inline void addPath(unsigned int a, unsigned int b,
                    const unsigned long long delt)
{
  Chain::processPath(
      a, b, [delt](unsigned int a, unsigned int b) { sroot->add(a, b, delt); });
}
inline unsigned long long sumPath(unsigned int a, unsigned int b)
{
  unsigned long long ret = 0;
  Chain::processPath(a, b, [&ret](unsigned int a, unsigned int b) {
    ret += sroot->sum(a, b);
  });
  return ret;
}
inline void addSubTree(const unsigned int x, const unsigned int rt,
                       const unsigned long long delt)
{
  Chain::processSubtree(x, rt, [delt](unsigned int a, unsigned int b) {
    sroot->add(a, b, delt);
  });
}
inline unsigned long long sumSubTree(const unsigned int x,
                                     const unsigned int rt)
{
  unsigned long long ret = 0;
  Chain::processSubtree(x, rt, [&ret](unsigned int a, unsigned int b) {
    ret += sroot->sum(a, b);
  });
  return ret;
}
