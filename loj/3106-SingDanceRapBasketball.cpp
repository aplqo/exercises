...
Number product(const unsigned int x[4], const unsigned int msk,
               const unsigned int n)
{
  ...
  for (unsigned int i = 0; i < 4; ++i)
    dft<false>(ConstArrayPtr{ifac, std::min(x[i] - msk + 1, n + 1)}, tmp[i],
               lgn);
...
Number count(const unsigned int n, const unsigned int x[])
{
  ...
  for (unsigned int i = 0; i * 4 <= n && i <= mn; ++i) {
    const Number p =
        C(n - 3 * i, i) * fac[n - 4 * i] * product(x, i, n - 4 * i);
