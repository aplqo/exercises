...
struct Vegetable {
  ...
  unsigned long long sell(const unsigned long long cnt)
  {
    ...
    soldCount += cnt;
    available -= cnt;
    if constexpr (first) currentPrice = price;
    return ret;
  }
...
unsigned long long greedyFromLast(const unsigned int m, const unsigned int maxP)
{
  ...
  for (unsigned int i = maxP; i; --i) {
    ...
    for (Vegetable** i = stk; i < top; ++i)
      if ((*i)->count > (*i)->soldCount) q.push(*i);
  }
  return ret;
}
void greedyFixAns(const unsigned int n, const unsigned int m,
                  const unsigned int maxP, unsigned long long ans)
{
  struct NodeLess {
    ...
    while (soldSum > maxSold) {
      ...
      if (cur->soldCount == 1) {
        ...
        const unsigned long long tp =
            std::min({soldSum - maxSold, cur->soldCount - 1});
