...
inline void addEdge(const unsigned int from, const unsigned int to, const unsigned int flow)
{
    static Edge* cur = ed;
...
unsigned long long isap(const unsigned int n, const unsigned int s, const unsigned int t)
{
...
{
    unsigned int r, c;
    inline bool operator==(const Point& other) const { return r == other.r && c == other.c; }
};
struct Step
{
    int dr, dc;
...
constexpr Step stepL[3] = {
    { -1, 0 }, { 1, 0 }, { 0, -1 }
};
constexpr Step stepR[3] = {
    { -1, 0 }, { 1, 0 }, { 0, 1 }
};
...
inline void linkList(const unsigned int vert[], const unsigned int n, const unsigned int cid, const unsigned int st[])
{
...
static std::tuple<unsigned int, unsigned int, unsigned int> buildGraph(const unsigned int r, const unsigned int c, const unsigned int n)
{
    offset = n;
    buildVertex(n);
    ...
    for (const Cube* i = cub; i < cub + n; ++i)
        ...
        {
            if ((i->c & 0x03) == 1)
                linkKeyEdge<false>(Point { i->r, i->c }, st);
        }
        else if ((i->c & 0x03) == 3)
