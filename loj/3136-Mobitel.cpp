...
unsigned int dp(const unsigned int r, const unsigned int s,
                const unsigned int n, const unsigned int cnt)
{
  static const auto minMul = [](const unsigned int base,
  ...
  for (unsigned int i = 1; i <= r; ++i) {
    ...
    for (unsigned int j = 1; j <= s; ++j)
      for (unsigned int k = 0; k < cnt; ++k) {
        if (i < r) addEq(nxt[j][minMul(rev[k], val[i + 1][j])], cur[j][k]);
        if (j < s) addEq(cur[j + 1][minMul(rev[k], val[i][j + 1])], cur[j][k]);
      }
    std::swap(cur, nxt);
  }
  return nxt[s][id[1]];
