...
static void init()
{
  ...
  for (unsigned long long i = 2; i <= maxb; ++i) {
    ...
    for (const unsigned int* j = primes;
         j < pcur && *j * i <= maxb && *j < mindiv[i]; ++j) {
      const unsigned int nxt = *j * i;
      mindiv[nxt] = *j;
      mu[nxt] = -mu[i];
    }
    if (mindiv[i] * i <= maxb) mindiv[mindiv[i] * i] = mindiv[i];
  }
...
long long coProduct(const unsigned int n,
                    const unsigned int t)  // sum of u(i)[gcd(i,t)==1]
{
  static std::unordered_map<std::pair<unsigned int, unsigned int>, long long,
                            Hash>
      mem;
  if (!n) return 0;
  if (t == 1) return DuJiao::dujiao(n);
  const std::pair<unsigned int, unsigned int> param(n, t);
  if (auto it = mem.find(param); it != mem.end()) return it->second;
  long long ret = 0;
                         ...
                         const unsigned int k)
{
  ...
  for (unsigned int i = 1; i <= n && i <= m;) {
    const unsigned int valN = n / i, valM = m / i,
                       nxt = std::min(n / valN, m / valM);
    ret += valN * (coProduct(nxt, k) - coProduct(i - 1, k)) *
