...
const unsigned int maxv = maxn * maxm * 2 + 2,
                   maxe =
                       (maxn * maxm * maxm * 2 + maxn * maxn * maxm + maxv) * 2;
...
inline void addEdge(const unsigned int from, const unsigned int to,
                    const unsigned int flow, const int cost)
{
  vertex[from].edge.push_back(
      Edge{from, to, cost, flow, {.id = vertex[to].edge.size()}});
  vertex[to].edge.push_back(
      Edge{to, from, -cost, 0, {.id = vertex[from].edge.size() - 1}});
}
    ...
    for (const Edge& i : vertex[cur.second].edge)
      if (i.flow && !vis[i.to]) {
        const unsigned int nxt =
            cur.first + vertex[i.from].h + i.cost - vertex[i.to].h;
        if (dis[i.to] > nxt) {
          dis[i.to] = nxt;
...
int augment(const unsigned int x, const unsigned int flow,
            const unsigned int sink)
{
  ...
  for (const Edge*& i = cur[x]; i <= &vertex[x].edge.back() && rst; ++i)
    if (i->flow && !vis[i->to] &&
        dis[x] + vertex[i->from].h + i->cost - vertex[i->to].h == dis[i->to]) {
...
std::pair<unsigned int, unsigned int> ssp(const unsigned int n,
...
void addPeople(const unsigned int desk, const unsigned int seat,
               const unsigned int l, const unsigned int r)
{
  const unsigned int cid = id(desk, seat);
  addEdge(source, cid, 1, 0);
  for (unsigned int i = l; i <= r; ++i)
    addEdge(cid, id(i, seat) + offset, 1,
            i > desk ? (i - desk) * 2 : (desk - i) * 2);
}
void readMatrix(const unsigned int n, const unsigned int m,
                unsigned int dest[][maxm])
{
  for (unsigned int i = 0; i < n; ++i)
