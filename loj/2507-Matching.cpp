...
inline bool match(const unsigned int prefix, const unsigned int cur, const unsigned int cval[])
{
    const unsigned int* const base = cval + cur - prefix;
    const Near npre = np[prefix];
    return (!npre.prev || base[npre.prev] < cval[cur]) && (!npre.next || cval[cur] < base[npre.next]);
}
