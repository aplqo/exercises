...
{
    if (FindUnion::isConnect(e.from, e.to))
        return false;
...
void conquer(const unsigned int leftL, const unsigned int leftR, const unsigned int rightL, const unsigned int rightR)
{
    if (leftR - leftL <= 1)
    {
        if (leftL != leftR)
            last[leftL] = findPos(max(leftL, rightL - 1), rightR);
        return;
    }
    ...
    if (Checkpoint ini; all_of(ed + leftL, ed + mid, tryAdd))
    {
        ...
        if (tryAdd(ed[mid]))
            conquer(mid + 1, leftR, mpos, rightR);
    }
