...
constexpr unsigned long long mod[cnt] = {
    static_cast<unsigned long long>(1e9 + 7),
    static_cast<unsigned long long>(1e9 + 9)};

class Number {
 public:
  Number() = default;
  Number(const unsigned long long v) : val1(v % mod[0]), val2(v % mod[1]){};
#define decl(x)                                                            \
  friend inline Number operator x(const Number l, const Number r)          \
  {                                                                        \
    return Number((l.val1 + mod[0]) x r.val1, (l.val2 + mod[1]) x r.val2); \
  }                                                                        \
  ...
  decl(-);
#undef decl
  inline Number operator-() const { return 0 - *this; }
