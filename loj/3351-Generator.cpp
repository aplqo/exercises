...
void createChange(ofstream& in, const unsigned int n, const unsigned int u,
  ...
  for (unsigned int i = 0; i < u; ++i) {
    ...
    do {
      ...
      v = disv(default_rnd);
             ...
             (ed[u].count(v) && (ed[u].size() > md || ed[v].size() > md)));
    in << u << " " << v << "\n";
    if (ed[u].count(v)) {
      ed[u].erase(v);
      ed[v].erase(u);
...
void createQuery(ofstream& in, const unsigned int n, const unsigned int u,
...
int main(int, char* argv[])
{
  ...
  ofstream in(argv[5]);
  in << n << " " << maxd << " " << u << " " << q << "\n";
  createArray(0, MayBe(atoi(argv[4]), maxh), n,
