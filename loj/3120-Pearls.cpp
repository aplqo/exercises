...
void ntt(const ConstArrayPtr a, const ConstArrayPtr b, Number dest[], const unsigned int lgn)
{
...
void buildLeast(const unsigned int n, const unsigned int d, const unsigned int lgd, Number dest[])
{
    static Number tmpa[maxd + 10], tmpb[maxd + 10];
...
void binomialInversion(const Number src[], const unsigned int d, const unsigned int lgd, Number dest[])
{
...
Number solve(const unsigned int d, const unsigned int n, const unsigned int m, const unsigned int lgd)
{
    static Number src[maxd + 10];
    buildLeast(n, d, lgd, src);
    binomialInversion(src, d, lgd, src);
    return std::accumulate(src + d - std::min(d, (n - 2 * m)), src + d + 1, Number { 0 });
}
