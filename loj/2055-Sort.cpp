...
{
public:
    inline void init(const unsigned int l, const unsigned int r, const unsigned int cnt);
    void build(const unsigned int l, const unsigned int r, const unsigned int pos);
...
inline void SegmentTree::init(const unsigned int l, const unsigned int r, const unsigned int cnt)
{
    lv = l;
    mv = (l + r) >> 1;
    rv = r;
...
void SegmentTree::build(const unsigned int l, const unsigned int r, const unsigned int pos)
{
    init(l, r, 1);
    if (r - l == 1)
        return;
...
void SegmentTree::print() const
{
    ...
    if constexpr (ord == Order::Decrease)
...
std::pair<SegmentTree*, SegmentTree*> SortedInterval::split(const unsigned int pos) const
{
    if (order == Increase)
    {
...
{
    auto it = std::prev(interval.upper_bound(SortedInterval { pos, 0, Order::Increase, nullptr }));
