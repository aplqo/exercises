...
inline void dp(const unsigned int n, const unsigned int m)
{
  ...
  const static auto Distance = [](unsigned int i, bool ls,
                                  bool ts) -> integer_t {
    return dis[clas[ls][i - 1]][clas[ts][i]];
  };
  const static auto Expect = [](unsigned int i,
                                function<real_t(bool)> f) -> real_t {
    return k[i] * f(true) + (1 - k[i]) * f(false);
  };
  const static auto Value = [](unsigned int i, unsigned int j, bool ls,
                               bool ts) -> real_t {
    return f[i - 1][j][ls] +
           (ls ? Expect(i - 1, bind(Distance, i, placeholders::_1, ts))
               : Distance(i, ls, ts));
  };
  ...
  for (unsigned int j = 1; j <= m; ++j)
    for (unsigned int i = 1; i <= n; ++i) {
      ...
      f[i][j][1] =
          min(Expect(i, bind(Value, i, j - 1, true, placeholders::_1)),
              Expect(i, bind(Value, i, j - 1, false, placeholders::_1)));
    }
}
...
int main()
{
  ...
  cin >> n >> m >> v >> e;
  init(v);
  for_each(clas[0] + 1, clas[0] + 1 + n,
           [](unsigned int& i) -> void { cin >> i; });
  for_each(clas[1] + 1, clas[1] + 1 + n,
           [](unsigned int& i) -> void { cin >> i; });
