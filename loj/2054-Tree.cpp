...
inline unsigned int getRoot(const unsigned int x) { return root[getFather(x)]; }
...
void initFinal(const unsigned int n, const unsigned int q)
{
  ...
  for (const Query* i = qry; i < qry + q; ++i)
    if (i->op == Query::Mark) ++cnt[i->x];
  for (unsigned int i = 1; i <= n; ++i)
    if (!cnt[i]) Disjoint::merge(fa[i], i);
}
void answerQuery(const unsigned int n, const unsigned int q)
{
  ...
  for (Query* i = qry + q - 1; i >= qry; --i) {
    if (i->op == Query::Mark) {
      --cnt[i->x];
      if (!cnt[i->x]) Disjoint::merge(fa[i->x], i->x);
    }
    else
