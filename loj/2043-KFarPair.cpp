...
inline unsigned long long distance(const Point& l, const Point& r)
{
  ...
  return sq(static_cast<long long>(l[0]) - r[0]) +
         sq(static_cast<long long>(l[1]) - r[1]);
}
struct Square {
  Point low, high;
  ...
  {
    return std::max({distance(p, low), distance(p, high),
...
void Node::updateFar(const Point pl)
{
  ...
  if (space.maxDistance(pl) <= heap.top()) return;
  lc->updateFar(pl);
  rc->updateFar(pl);
}
