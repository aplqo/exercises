...
long long exgcd(const long long a, const long long b, long long& x, long long& y) noexcept
{
    if (!b)
    {
        x = 1;
...
Equation solveEqu(const long long a, const long long b, const long long c) // Solve ax + by = c returns x
{
    long long x, y;
