...
inline void countSquare(const unsigned int x, const unsigned int f,
                        const unsigned int cnt)
{
  if (f)
    ans += static_cast<unsigned long long>(vertexNum - siz[x]) * siz[x] *
           (cnt - 2);
  for (auto j = tree.head[x]; j; j = j->pre)
    if (j->to != f)
      ans += static_cast<unsigned long long>(vertexNum - siz[j->to]) *
             siz[j->to] * (cnt - 2);
}
