...
{
  static const auto modInv = [](unsigned long long x) {
    return quickPow(x, mod - 2);
  };
  static const auto minus = [](unsigned long long a, unsigned long long b) {
    return (a + mod - b) % mod;
  };
  static const auto multiply = [](unsigned long long a, unsigned long long b) {
    return (a * b) % mod;
  };
  ...
  return multiply(
      cRt[1], multiply(cl, multiply(minus(quickPow(n, 2 * d), quickPow(e, d)),
...
int main()
{
  ...
  dfsRooted(1, 0);
  dfsChangeRt(1, 0);
  cout << (loseRt[1] ? Ld_1v(n, d)
                     : (quickPow(n, 2 * d) + mod - Ld_1v(n, d)) % mod)
       << "\n";
  return 0;
}