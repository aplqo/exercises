...
void constructOdd(const unsigned int n, const unsigned int m,
    ...
    if (full[i]) {
      ...
      if ((i - last) & 0x01) {
        for (unsigned int j = last + 1; j < i; ++j) {
          for (unsigned int k = n - !((j - last) & 0x01);
               k && a[j] && !select[k][j - 1]; k -= 2) {
            select[k][j] = true;
            --a[j];
          }
          for (unsigned int k = ((j - last) & 0x01) + 1; k <= n && a[j];
               k += 2) {
            select[k][j] = true;
            --a[j];
          }
