...
Number countInvFood(const unsigned int n, const unsigned int t)
{
    ...
    static const auto index = [](const unsigned int col, const int val) -> Number& {
    ...
    for (int i = 1; i <= n; ++i)
    {
        ...
        for (int j = -i; j <= i; ++j)
            index(i, j) = index(i - 1, j) + index(i - 1, j - 1) * a[i][t] + index(i - 1, j + 1) * (sum[i] - a[i][t]);
    }
...
int main()
{
    ...
    out << countNoRestrict(n) - countInvalid(n, m) << "\n";
    return 0;
}