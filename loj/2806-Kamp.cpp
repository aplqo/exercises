...
pair<unsigned long long, unsigned int> dfsPath(const unsigned int x,
                                               const unsigned int f)
{
  pair<unsigned long long, unsigned int> ret(0, x);
  for (const Edge* i = head[x]; i; i = i->pre) {
    if (i->to == f || !key[i->to]) continue;
...
void getans(const unsigned int n, const pair<unsigned int, unsigned int> r)
{
  ...
  for (unsigned int i = 1; i <= n; ++i)
    if (key[i]) {
      ans[i] =
          esum - max(lca::distance(i, r.first), lca::distance(i, r.second));
      dfsNotKey(i, 0);
    }
}
