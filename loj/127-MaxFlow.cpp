...
private:
    void pushRange(const unsigned int source, const unsigned int sink, const Edge* beg, const Edge* end);
} vertex[maxn + 10];
struct VertexLess
{
...
inline void addEdge(const unsigned int from, const unsigned int to, const Flow flow)
{
...
void Vertex::relabel()
{
    ...
    for (const Edge& i : edge)
        if (i.flow)
            mn = std::min(mn, vertex[i.to].h);
    if (mn != UINT_MAX)
        h = mn + 1;
}
template <bool ignCheck>
inline void pushEdge(const Edge* e, const Flow delt, const unsigned int source, const unsigned int sink)
{
...
inline void Vertex::pushRange(const unsigned int source, const unsigned int sink, const Edge* beg, const Edge* end)
{
...
void Vertex::push(const unsigned int source, const unsigned int sink)
{
    ...
    if (exceed)
        pushRange(source, sink, begin, cur);
}
void Vertex::init()
{
    if (!edge.empty())
...
Flow hlpp(const unsigned int n, const unsigned int source, const unsigned int sink)
{
    ...
    for (const Edge& i : vertex[source].edge)
        if (i.flow)
            pushEdge<true>(&i, i.flow, source, sink);
    while (!heap.empty())
    {
