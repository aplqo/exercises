...
namespace OrigGraph
{
    ...
    inline void addEdge(const unsigned int id, const unsigned int from, const unsigned int to)
    {
        static Edge* cur = ed;
...
namespace Tree
{
    ...
    template <Direct d>
    inline Edge* addDirEdge(const unsigned int from, const unsigned int to, Direct* p)
    {
    ...
    {
        Edge *const e1 = addDirEdge<R>(from, to, p), *const e2 = addDirEdge<L>(to, from, p);
        e1->rev = e2;
        e2->rev = e1;
    }
    namespace LCA
    ...
    void tarjan(const unsigned int x, const OrigGraph::Edge* fe)
    {
        ...
        for (auto i = OrigGraph::head[x]; i; i = i->pre)
        {
            ...
            if (!dfn[to])
            {
                ...
                if (dfn[x] < low[to])
                    i->isBridge = i->rev->isBridge = true;
            }
            else
...
void applyConstraint(const unsigned int n, const unsigned int m, const unsigned int p)
{
...
int main()
{
    ...
    transform(direct, direct + m, ostream_iterator<char>(cout), [](Direct i) { return ch[i]; });
    cout.put('\n');
    return 0;
}