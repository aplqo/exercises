...
int main(int argc, char* argv[])
{
    ...
    ofstream in(argv[5]);
    in << t << endl;
    uniform_int_distribution<unsigned long long> disa(1, MayBe(atoll(argv[2]), maxn)), disk(0, MayBe(atoll(argv[3]), maxk));
    for (unsigned int i = 0; i < t; ++i)
        in << disa(default_rnd) << " " << disa(default_rnd) << " " << disk(default_rnd)
           << " " << MayBe(atoi(argv[4]), GetRandom(1, maxm)) << endl;
    return 0;
}