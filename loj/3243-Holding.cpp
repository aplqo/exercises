...
template <int step>
void dp(const int outBegin, const int outEnd, const int inBegin,
        const int inEnd, const int n, Array res)
{
  for (int in = inBegin; in != inEnd; in += step)
    for (int out = outBegin; out != outEnd; out += step)
      for (int sp = 0; sp <= n * n / 4; ++sp) {
        res[in][out][sp] =
            max(res[in - step][out][sp], res[in][out - step][sp]);
        if (sp >= abs(in - out))
          res[in][out][sp] = max(
              res[in][out][sp],
              res[in - step][out - step][sp - abs(in - out)] + a[in] - a[out]);
      }
}
unsigned int dp(const unsigned int n, const unsigned int l,
