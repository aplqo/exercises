...
{
  if (this->d != r.d) return this->d < r.d;
  for (int i = d - 1; i >= 0; --i)
    if (this->digit[i] != r.digit[i]) return this->digit[i] < r.digit[i];
  return false;
}
...
{
  return max(fType(1) / a.b, fType(a.a) / b.b) <
         max(fType(1) / b.b, fType(b.a) / a.b);
}
int main()
{
  unsigned int n;
