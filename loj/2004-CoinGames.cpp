...
namespace Hash
{
    ...
    inline unsigned long long substrHash(const unsigned long long hash[], const unsigned int l, const unsigned int r)
    {
        return hash[r] - hash[l - 1] * powBase[r + 1 - l];
    }
    inline bool equal(const unsigned long long pre[], const unsigned long long suf[], const unsigned int n, const unsigned int l)
    {
    ...
    for (unsigned int i = 1; i <= n; ++i)
    {
        ...
        for (unsigned int j = 1; j <= n; ++j)
            for (unsigned int k = 1; k <= m; ++k)
                if (Hash::equal(hash[i], hash[j], m, k))
                    equ[i][j] += pow2[k];
    }
    for (unsigned int i = 1; i <= n; ++i)
        equ[0][i] = 1;
