...
void VirtTree::build(const unsigned int n, unsigned int v[])
{
  ...
  sort(v, v + n,
       [](unsigned int a, unsigned int b) { return dfn[a] < dfn[b]; });
  *(top++) = 1;
...
{
  const auto dist = [](unsigned int x, const unsigned int f,
  ...
  for (unsigned int i = lg[dep[x] - dep[father]]; i;) {
    ...
    const unsigned int df = dist(m, father, f[father].first),
                       dx = dist(x, m, basx);
...
unsigned int dfsVirtAns(const unsigned int x, const unsigned int father)
{
  ...
  const unsigned int pos =
      f[x].second == f[father].second ? x : findPos(x, father);
...
int main()
{
  ...
  }
  {
    ...
    for (unsigned int i = 0; i < q; ++i) {
      ...
      query(k, qbuf);
      transform(orig, orig + k, ostream_iterator<unsigned int>(cout, " "),
                [](unsigned int i) { return ans[i]; });
      cout << "\n";
    }
  }
  return 0;
