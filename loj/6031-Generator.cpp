...
void createStr(ofstream& in, const unsigned int n, const char* str, const size_t len)
{
    uniform_int_distribution<unsigned int> dis(0, len - 1);
    generate_n(ostreambuf_iterator<char>(in), n, [&dis, str]() { return str[dis(default_rnd)]; });
}
...
void createQuery(ofstream& in, const unsigned int m, const unsigned int q, const unsigned int k, const char* ch, const size_t len)
{
...
int main(int, char* argv[])
{
    ...
    const unsigned int k = GetRandom(1u, min<unsigned int>(sl, MayBe(atoi(argv[4]), maxk)));
