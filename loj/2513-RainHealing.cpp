...
static void initProbably(const unsigned int n, const unsigned long long m,
...
bool gaussian(const unsigned int n)
{
  ...
  for (unsigned int i = n; i; --i) {
    if (equ[i][n + 1].notZero() && !equ[i][i].notZero()) return false;
...
std::pair<bool, Number> solve(const unsigned int p, const unsigned int n,
                              const unsigned long long m,
                              const unsigned long long k)
{
  if (p && (!k || (!m && k == 1))) return std::make_pair(false, Number{0});
