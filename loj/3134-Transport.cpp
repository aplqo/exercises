...
inline void addEdge(const unsigned int from, const unsigned int to, const int dis)
{
    static Edge* cur = ed;
...
void dfsPoint(const unsigned int x, const unsigned int f, const unsigned int n, unsigned int& pos, unsigned int& mxv)
{
    siz[x] = 1;
    unsigned int sz = 0;
...
void dfsSubtree(const unsigned int x, const unsigned int f, long long sum, const long long mx)
{
    rootTo.push_back(mx);
    subTo.push_back(mx);
    sum -= a[x];
    for (const Edge* i = head[x]; i; i = i->pre)
    {
        ...
        if (to != f && !vis[to])
            dfsSubtree(to, x, d, max(mx, d));
    }
}
void dfsAddVertex(const unsigned int x, const unsigned int f, const long long sum, const long long mx)
{
    if (mx >= 0)
    {
    ...
    for (const Edge* i = head[x]; i; i = i->pre)
    {
        ...
        if (to != f && !vis[to])
            dfsAddVertex(to, x, sum + d, min(d, mx + d));
    }
}
...
void conquer(const unsigned int x)
{
    ...
    for (const Edge* i = head[x]; i; i = i->pre)
        if (!vis[i->to])
            conquer(treeCenter(i->to, siz[i->to]));
}
