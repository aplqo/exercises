        ...
        while (x)
        {
            ...
            if (x != bottom[ch.color])
                if (unsigned int chl = findChild(x, bottom[ch.color]); chl != from)
                    subtreeAdd(chl, 1);
            if (x != ch.bottom)
                root->modify(dfn[x] + 1, dfn[ch.bottom] + 1, Color { rev[dfn[x] + 1], ch.bottom, ch.color });
            root->modify<Color>(dfn[ch.top], dfn[x] + 1, Color { top[x], last, color });
            from = ch.top;
            x = fa[ch.top];
            if (ch.top == top[ch.top])
                last = x;
        }
    }
    inline unsigned int countPath(const unsigned int x, const unsigned int y)
    {
        return root->get<int>(dfn[x]) + 1 + root->get<int>(dfn[y]) - 2 * root->get<int>(dfn[lca(x, y)]);
    }
}
