...
public:
    line(const point a, const point b) noexcept
    {
        ...
        this->b = (b.second * a.first * a.first - a.second * b.first * b.first) / div;
        if (!(this->a < 0))
            return;
        fail = false;
    }
...
private:
    real_t calc(real_t x) const noexcept
    {
        return x * x * a + x * b;
    }
...
namespace aplog
{
    ...
    void PrintLog(unsigned int n)
    {
        ...
        {
            logs << "Info f[i] = " << f[i] << " select=(" << sel[i].first << "," << sel[i].second << ")" << endl;
        }
        logs << endl;
    }
}
...
inline void solve()
{
    ...
    for (res_t i = 1; i < (res_t(1) << n); ++i)
    {
        ...
        for (unsigned int j = s + 1; j < n; ++j)
            if (cov[s][j])
                f[i] = min(i, i & ~cov[s][j], make_pair(s, j));
