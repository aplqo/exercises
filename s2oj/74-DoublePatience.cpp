...
struct state {
  ...
  {
    return dat[pos[0]][pos[1]][pos[2]][pos[3]][pos[4]][pos[5]][pos[6]][pos[7]]
              [pos[8]];
  }
  unsigned int pos[9] = {};
};
...
{
  if (!s.remain()) return s[f] = 1;
  if (s[vis]) return s[f];
  s[vis] = true;
  unsigned int p = 0;
  for (unsigned int i = 0; i < 9; ++i) {
    if (s.pos[i] > 3) continue;
    for (unsigned int j = i + 1; j < 9; ++j) {
      if (s.pos[j] > 3 || dat[i][s.pos[i]] != dat[j][s.pos[j]]) continue;
      ++p;
