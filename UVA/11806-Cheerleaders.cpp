...
class Number {
 public:
  Number() = default;
  Number(unsigned int i) : val(i % mod) {}
#define decl(x)                                                    \
  ...
  decl(-);
#undef decl
  inline explicit operator unsigned int() { return val; }
...
{
  const auto sn = [&k](const unsigned int n, const unsigned int m) {
    return c[n * m][k];
  };
  return static_cast<unsigned int>(
      sn(n, m) - (2 * sn(n - 1, m) + 2 * sn(n, m - 1)) +
      (4 * sn(n - 1, m - 1) + sn(n, m - 2) + sn(n - 2, m)) -
      (2 * sn(n - 1, m - 2) + 2 * sn(n - 2, m - 1)) + sn(n - 2, m - 2));
}
int main()
{
#ifndef APTEST
