...
static void eular(const unsigned int n)
{
    for (unsigned int i = 2; i <= n; ++i)
    {
        ...
        for (const unsigned int* j = primes; j < pcur && static_cast<unsigned long long>(*j) * i <= n && *j <= mindiv[i]; ++j)
            mindiv[*j * i] = *j;
    }
}
unsigned int count(const unsigned int n)
{
    ...
    for (const unsigned int* r = upper_bound(primes, pcur, n) - 1; l <= r; --r, ++ret)
    {
