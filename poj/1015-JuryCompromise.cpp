...
#include <iostream>
using namespace std;
const int inf = INT_MAX, ninf = INT_MIN / 2;
...
int GetDiff(const unsigned int n, const unsigned int m)
{
  int ans = 0, po = inf;
  for (int i = -mi; i <= ma; ++i) {
    if (ind(f, m, i) < 0) continue;
...
void solve(const unsigned int id)
{
  ...
  cout << "Jury #" << id << endl;
  cout << "Best jury has value " << (dif + ind(f, m, dif)) / 2
       << " for prosecution and value " << (ind(f, m, dif) - dif) / 2
       << " for defence:" << endl;
  GetAnswer(n, m, dif);
  for (unsigned int i = 1; i <= n; ++i)
    if (ans[i]) cout << " " << i;
