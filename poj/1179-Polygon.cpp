...
const num_t ninf = LLONG_MIN, inf = LLONG_MAX;
const int maxn = 50;
    ...
    for (unsigned int l = 0; l <= n * 2 - d; ++l) {
      ...
      for (unsigned int k = l; k < r; ++k) {
        if (v[k + 1].typ) {
          ...
          f[MAX][l][r] = max(f[MAX][l][r], max(cma, cmi));
          f[MIN][l][r] = min(f[MIN][l][r], min(cma, cmi));
        }
        else {
...
int main()
{
  ...
  cout << ans << endl;
  if (f[MAX][0][n - 1] == ans) cout << "1 ";
  for (unsigned int i = 1; i < n; ++i)
    if (f[MAX][i][i + n - 1] == ans) cout << i + 1 << " ";
  return 0;
}